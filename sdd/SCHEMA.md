...
| `heading_band_scene` | sprite path | — | A single curated image drawn **once**, fitted to the band's height with its width from the source's aspect, anchored to the band's **right** edge and clipped to it. Composites **over** the fill/gradient rather than replacing it — the opposite of `heading_band_sprite` beside it, and a theme may state both. A scene alone is a band. ⚠️ Not on the PDF page: a paginated band is several abutting rects with no single right edge to anchor one scene to. |
| `heading_band_scene_anchor` | `top-left` \| `top-right` \| `bottom-left` \| `bottom-right` | — | WHERE the level's `heading_band_scene` hangs — **and how it is sized**, which is one decision rather than two: pinned to a corner the scene is drawn at its **own size** (scaled by zoom), where unstated it keeps the fitted right-edge rendering the row above describes. A corner gives a picture no second dimension to be fitted to, so an anchor and a fit are alternatives, not a pair; that is why there is no fifth `right` value and no separate size key. Overflow runs off the edge the anchor is *not* on and is clipped. Ignored where the level states no scene. ⚠️ Not on the PDF page — it moves a picture that sink does not draw. |
...
| `heading_marker_sprite` | sprite path | — | An icon drawn immediately **after** the heading's text — on the **last** display row of a heading that soft-wraps. Placed by Pango as a shape rather than at a computed x. |
| `heading_marker_size` | `i32` | `22 · 17 · 14 · 12 · 11` | The marker's box **height** in design px at zoom 1.0; the width follows the sprite's aspect, so a non-square marker is not stretched. Defaults are the cap height of the shipped `heading_scale` ladder, which is what puts the box bottom on the baseline and its top at cap height. Art for this key must be **bottom-flush and full-height** — padding at the bottom lifts the icon off the baseline, padding at the top makes it read small. Clamped `0`–`400`. Ignored where the level states no marker. |
...
| `code_inline_bg` | colour | derived | Inline code-span background — **the PAGE's**, and only the page's. Every other surface the preview draws behind text (a heading band per level, a quote panel, a table header's fill) derives its own chip from that surface, tinted 8% toward the ink that surface carries, because a colour chosen against the page says nothing about a band drawn over it (TDD 18.61, ScrAP-356). A surface whose colour is unknown — a level banded with a tile and no fill stated — carries no chip at all. |
...
| `blockquote_bg_sprite` | sprite path | — | A sprite **tiled at its natural size** across the quote panel, in place of `blockquote_bg`. **Outranks the fill**, which is not painted underneath it — a transparent tile would let it bleed through. ⚠️ **Stated alone it still makes a panel**: the fill is not a precondition, exactly as `heading_band_sprite` alone makes a band. Does **not** nest, matching the fill: the grid is laid once from the outermost level (TDD 2.11b). Tiles at natural size, so it does not scale with zoom. On the PDF page the grid is anchored to the PAGE rather than to each line's rect, so a quote drawn line by line carries one continuous pattern. |
| `blockquote_scene` | sprite path | — | A single curated image drawn **once** at a fixed size in the quote panel's **bottom-right** corner, over `blockquote_bg` and clipped to the panel. Scales with zoom. A longer quotation therefore reveals *more* of the scene rather than magnifying it — a panel's height is the quote's length, so fitting to it would balloon the picture. Does **not** nest, matching the fill. ⚠️ Not on the PDF page, for the reason `heading_band_scene` gives. |
| `blockquote_bar_width` | `i32` | `3` | Clamped `0`–`400`. With `blockquote_text_gap` it is also the **per-level step**: a nested quote indents, and draws its own bar, `blockquote_bar_width + blockquote_text_gap` further in per depth, to a cap of 6 levels. |
| `blockquote_text_gap` | `i32` | `10` | Bar → quoted text. Clamped `0`–`400`. |
| `blockquote_bg` | colour | — | A panel behind quoted text. Absent unless stated: a quote sits on the page background, as it always has. Independent of `blockquote_bar_color` — an accent bar and a panel are two decisions, and a themed bar seeds no panel. ⚠️ **Does not nest.** A quote inside a quote inherits its parent's fill rather than painting a second panel over it, so depth is carried by the bars alone (TDD 2.11b). A translucent value is therefore composited once however deep the nesting goes. |
...
| `table_head_scene_anchor` | `top-left` \| `top-right` \| `bottom-left` \| `bottom-right` | — | The header cell's twin of `heading_band_scene_anchor`, with the same "the anchor is also the size rule" reading: pinned to a corner of the **cell**, drawn at its own size, clipped to it. Ignored where `table_head_scene` is unstated. ✅ Reaches the PDF page, as the scene it governs does. |
