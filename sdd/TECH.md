...
| `tags.rs` | Owns the preview's `GtkTextTag` set and its style setup — the GTK half. Its child `spec` owns the theme→tag DECISIONS as display-free values (the quote-ink floor's condition, the band inset's per-level gate, the design-time→pixel arithmetic), so they are assertable without a live buffer. |
