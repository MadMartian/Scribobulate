...
| `theme/` | Owns the reading-theme engine, split by stage so each piece is readable and testable without the rest. `keys` is the **registry**: the one enumeration of the vocabulary a themes file may speak, carrying each key's type, how many values it has (one, one per heading level, one per bullet depth) and whether it names a sprite — so validation, the merge, the sprite walk and the per-level fallback all read one list rather than four that can drift. `spec` is the file model (a validated map, and the warning an unknown key earns); `sources` is the two-source resolution walk, identical for every kind and levelling — a narrower spelling wins within a theme, the source wins between themes; `value` holds the authored value types, each parsed or refused; `model` holds the resolved `Theme` every consumer reads; `resolve` holds the one function that walks the resolution order (the floors and clamp ranges live on the keys themselves); `decor` is the **decoration vocabulary's precedence** — the one place a sprite's rank over its flat sibling is decided, in three shapes (two-way fill, three-way band, marker), each a struct of ordered candidates so a consumer that cannot produce the winner degrades to the next rung rather than erasing the decoration; `mod` holds the search path, the built-in/user merge and the active-theme cell. The engine carries no per-theme knowledge, and states the closed decoration vocabulary (`sdd/PLAN.preview-decoration.md`, ratified) — a theme selects from decorations the engine already knows how to draw and states their appearance; it never describes new drawing. Parsing is also where every sprite reference is answered against its file's origin (`sprite.rs`), so a spec that has escaped it carries sources rather than strings and `Theme::resolve` stays pure. |
