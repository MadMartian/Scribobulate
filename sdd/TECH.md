...
| `window/foldsplice.rs` | Owns resolving which pane, which source text and which settings a fold toggle applies to, and handing off to `preview/splice`. Deliberately holds no splice mechanics — it is the window layer's half, and it splits from its sibling `window/foldreveal.rs` on the READER'S INTENT rather than on mechanism: this is the reader toggling the block in front of them and expecting to stay put. |
| `window/foldreveal.rs` | Owns making content inside a collapsed disclosure reachable — expand the chain that hides it, re-render, then act on the render that results. The other intent: the reader has asked to be moved somewhere else (an outline heading, a find match), so a full re-render plus a navigation is what they want. Two features would otherwise each invent the same three steps in the same order. |
| `preview/splice/regionwriter.rs` | Owns the ORDER a region render must be driven in — view and seed before the write offset, the offset before any content, `finish_region` exactly once — as a two-state typestate rather than as prose. Getting one of those wrong is not a crash: it produces a well-formed buffer with every map below the splice silently offset, which is the one failure this route has to be structurally safe from. |
| `codeview/disclosurebands.rs` | One painter, in `codeview/`'s per-decoration shape: the band behind a disclosure's summary line. What differs from its heading-band sibling is how the decoration is RESOLVED (flat, versus stated per level); the band itself is painted by `codeview/bandpaint.rs`. |
| `codeview/bandpaint.rs` | Owns painting ONE line-wide band at the content column — the single arithmetic behind both the heading band and the disclosure summary band. Shared because the two painters had already diverged over sprite decoding while being the same rect, radius and sprite→gradient→flat precedence. |
| `codeview/` | Owns `CodePreviewView`, the preview's `GtkTextView` subclass that self-draws code-block backgrounds and their copy buttons, task checkboxes, list markers, heading bands, blockquote panels and bars, and the right-margin annotation chips. `mod.rs` holds the subclass, its state and its setters; each decoration's paint is a sibling module and the order they run in is `decorplan`'s. |
...
| `codeview/copybutton.rs` | Owns how a code block's copy button is *drawn*. Where it goes is not here — that is `affordance.rs`, so the placement is settled without a display. |
...
| `codeview/paint.rs` | Owns the decoration paint's per-frame context — the viewport, the content column, and the one quote extent the panel and the accent bar are both drawn from — and the dispatch that walks `decorplan::PAINT_ORDER`. A total `match` on the step, so a decoration added to the order does not compile until it has a painter. |
| `codeview/quotes.rs`, `bands.rs`, `cards.rs`, `listmarkers.rs`, `chips.rs` | One painter each: the blockquote panel and accent bars, the heading band (which resolves per level and hands the band itself to `bandpaint.rs`), the code-block card, the list gutter's per-item placement, and the right-margin annotation chips. Each owns only the live geometry read and the paint; what it decides without a display is in `decorplan`. `quotes.rs` draws ONE BAR PER NESTING LEVEL but only ONE panel, from the outermost — the asymmetry is the contract, not an oversight (TDD 2.11b). |
| `codeview/pending.rs` | Owns firing a programmatic navigation's armed popover request — the paint's own completion event, since GTK offers no "that paint happened" signal. A state machine rather than a painter: what remains here is the two live readings (has the scroll landed, did the chip paint), and the precedence between them is `decorplan::pending_open_gate`. |
| `codeview/gutter.rs` | Owns the list gutter's SHAPES and its pure column arithmetic — the marker glyphs, the checkbox rect, and the `li-{depth}` content margin the tags and the paint must agree on. Its live-geometry half is `codeview/listmarkers.rs`. |
| `decorplan.rs` | Owns the preview's paint PLAN: the ordered list of steps `snapshot_layer` iterates and the layer each belongs to, plus the decisions the paint takes that need no display — the viewport gates, the band's corner-radius clamp and level slot, the copy button's reveal rule, and the pending-popover precedence. Display-free, so the compositing order between two overlapping decorations is assertable without painting pixels; `codeview::ordertests` proves the pixels agree with it. |
| `readingpos.rs` | Owns `DocPosition` — the one document coordinate the editor and preview panes both resolve — and the conversions each pane makes to and from it. Display-free, so the split-pane projection and the view-mode hand-off share one implementation of the arithmetic rather than each carrying its own. |
| `renderer/` | Owns the low-level rendering mechanics that turn the parsed Markdown event stream into buffer text and tags. |
| `renderer/image.rs` | Owns the display-free image decisions: the broken-image placeholder's reason string, and the extent arithmetic deciding how large an image is drawn — the zoom term, the fit to the content column, and the pixel budget a vector re-render may allocate. A module rather than inline arithmetic because it has **two** appliers that must not disagree: `renderer/start.rs` sizes a picture at render time and `codeview::size_allocate` re-fits it on every width change. |
| `renderer/rawhtml/` | Owns the raw-HTML **allowlist** — the single statement of which elements survive sanitisation-by-omission — and which literal text a block contributes to the page. One owner because the permitted set is a security posture, and it feeds two scanners and two sinks. |
| `renderer/rawhtml/tags.rs` | Owns tag LEXING: where one tag ends and what attributes it carries, both decided by quote state. Separate from the allowlist because it is the answer three scanners need and each had been deriving it from a delimiter search, which reads a `>` out of an attribute value and re-scans the tag's own tail as markup. |
| `renderer/disclosure.rs` | Owns two display-free scans: a raw-HTML fragment's `<details>`/`<summary>` tags and the summary's label, and the whole document's `<details>`↔`</details>` **pairing**. The renderer's walk is forwards and one-pass, so "is this block ever closed?" — which decides whether it may collapse at all — has to be answered before the walk begins. |
| `tags.rs` | Owns the preview's `GtkTextTag` set and its style setup — the GTK half. Its child `spec` owns the theme→tag DECISIONS as display-free values (the quote-ink floor's condition, the band inset's per-level gate, the design-time→pixel arithmetic, and the two block INDENTS — `list_indent_px` and `quote_indent_px` — which `renderer/` also reads, so the tag that applies a margin and the anchored-child bound that must clear it cannot round it differently), so they are assertable without a live buffer. |
| `span.rs` | Owns `BufferSpan`, the named character-offset range the renderer uses to record where each block landed, and `QuoteSpan`, which pairs one with a blockquote's nesting depth so a painter can draw a bar per level without re-deriving the depth. |
| `affordance.rs` | Owns the display-free geometry of the preview's drawn, clickable controls: where a code block's copy button sits in its card, and the one point-in-rectangle test every such control's hit-box is read through. Display-free so both are settled by unit test, leaving `codeview` the paint and the coordinate conversion — the same split `keynav` makes with `codeview::navkeys`. |
...
| `macwordnav.rs` | macOS only (`#[cfg]`-gated at its declaration in `lib.rs`/`gtk_suite.rs`). Owns Option+Left/Right word navigation in the main editor: a key controller that answers the keystroke itself and pre-empts `GtkSourceView`'s own `move-words` class binding (a word-*transposition* edit, default-bound to the same key, that GTK does not expose through any binding table `keynav.rs` mirrors). Decision half (`word_movement`) is display-free and unit-tested; the wiring is the one place it is attached, in `window/tabs/lifecycle.rs`'s `build_tab_editor`. |
| `copymap.rs` | Owns display-free copy-as-Markdown for the preview. |
| `export/` | Owns exporting a document to a presentation format — HTML to share, PDF to record. A **second consumer of the document source**, in `copymap`'s shape and for the same reasons: an export is a function of the source and the same normalised event stream the preview is built from, never of the preview widget, so it holds on a tab that was never rendered, in any view mode, on an unsaved buffer, with no display. `doc`/`walk` build one `ExportDoc`, `html` and `pdf` are its two sinks, and `paginate` decides page boundaries from measured heights while `pdftable` decides a table's column widths from measured cell widths — the same seam twice, and for the same reason: both are arithmetic over numbers Pango supplied, so both are settled by unit test rather than by a human holding a ruler against a viewer. **One pipeline, two sinks**: every decision about what a construct *is* was made upstream of both, so they agree with the preview by construction rather than by vigilance. `pdf/` is the GTK-touching corner and is deliberately a thin adapter — measurement and ink, no decisions — so everything else sits inside the coverage gate. It is split along the **toolkit boundary** rather than by size: `pdf/geometry` (page arithmetic) and `pdf/decide` (what a construct becomes — list markers, heading-scale index, column count, image splitting) need no toolkit and are unit-tested headlessly, while `pdf/measure` (+ `measure/table`, the one construct measured twice) and `pdf/ink` are the halves that genuinely need Pango and cairo. That boundary is the whole point of the split: as one 1917-line file, every one of those toolkit-free decisions could only be reached by building a document and a Pango context and running the entire measurement pass. |
| `format/` | Owns the display-free Markdown formatting core behind the Format commands. |
| `outline/` | Owns the display-free document-outline model, and `HeadingSite` — where each heading the *source* declares is reachable in a given render, which is not the same list as the headings that reached the buffer. `tree` holds the flat-list → forest fold and the scroll-spy's ancestor-chain lookup, split out of `mod.rs` once `extract_headings` grew its own exhaustive `Event` match (POLICY's 500-line soft limit). |
| `fold.rs` | Owns the display-free fold model: which disclosure blocks are collapsed, keyed on source offset. Read by the renderer as an input; holds no GTK type. |
...
| `limits.rs` | Owns the input-cost limits a document is admitted under — the construct-nesting bound and the size/file-type admission test — and is the single source of truth for their values, which POLICY § Input limits deliberately does not restate. It also holds the one bound that is **not** about hostile input: the pixel budget a vector image's zoom re-render may allocate, an expense the application chooses rather than one a document imposes. |
| `imagecache/` | Owns whether a remote image needs fetching at all: a process-wide, URL-keyed store of decoded textures in front of `imagefetch`'s single network path, with the eviction and negative-caching policy as a display-free core. It exists because a render is no longer a rare event — a disclosure fold toggle re-walks the whole document, and the fetch behind it is synchronous on the main thread. |
| `clipboard.rs` | Owns what the **editor** puts **on** a clipboard: plain text, never GTK's default rich `GtkTextBuffer`. That is what keeps a same-application paste to a SINGLE `insert-text` emission — rich content is re-inserted one chunk per syntax-highlight tag toggle, and a toggle inside a `\r\n` is the mechanism behind ScrAP-312. CLIPBOARD is covered on both panes (this module overrides `copy-clipboard`/`cut-clipboard` on the editor; the preview publishes plain text by its own separate route in `preview/interactions.rs`). PRIMARY is **not** taken over on either pane — GTK republishes it on every selection change, and the publisher-side takeover this module once used could not survive the preview's own re-render-swaps-its-buffer behaviour, so it was replaced with a consumer-side fix: the editor's middle-click paste reads PRIMARY as text rather than as a buffer, which is safe against *any* publisher's content, including the preview's still-un-taken-over rich one. ScrAP-313's PRIMARY-selection buffer leak is a GTK-core defect this module cannot reach either way. |
| `lineendings.rs` | Owns the one rule this application holds about line separators — **a document buffer never contains a lone carriage return** — applied at both doors documents arrive by: `docio`'s readers and `window::actions::load_into_editor` for files, and an `insert-text` hook for the clipboard. Never at a parse site, and `\r\n` is deliberately untouched. The **repair itself** is display-free (`&str`/bytes in, `Cow` out, unit-testable headlessly); the module also carries the GTK-bound half that ARMS it — `new_editor_buffer` and `wire_paste_normalization` construct and wire a `sourceview::Buffer` — so the module as a whole is not display-free and is deliberately not excluded from the coverage scope. The hook does not make the failure impossible, it merely relies on two conditions elsewhere in the tree that currently prevent it — both are named at the function, one asserted by a test and one **convention-only, declared as such**: there is no gate on it, and the function says so rather than implying one. ScrAP-312 records why the route that WOULD make it impossible is unavailable. |
| `theme/` | Owns the reading-theme engine, split by stage so each piece is readable and testable without the rest. `keys` is the **registry**: the one enumeration of the vocabulary a themes file may speak, carrying each key's type, how many values it has (one, one per heading level, one per bullet depth) and whether it names a sprite — so validation, the merge, the sprite walk and the per-level fallback all read one list rather than four that can drift. `spec` is the file model (a validated map, and the warning an unknown key earns); the registry also carries the one exception the diagnostics need — which keys a surface reads in their **bare** form regardless of levelling, so a report that a key is shadowed at every level cannot fire on one that still applies; `sources` is the two-source resolution walk, identical for every kind and levelling — a narrower spelling wins within a theme, the source wins between themes; `value` holds the authored value types, each parsed or refused; `model` holds the resolved `Theme` every consumer reads; `resolve` holds the one function that walks the resolution order (the floors and clamp ranges live on the keys themselves); `decor` is the **decoration vocabulary's precedence** — the one place a sprite's rank over its flat sibling is decided, in three shapes (two-way fill, three-way band, marker), each a struct of ordered candidates so a consumer that cannot produce the winner degrades to the next rung rather than erasing the decoration; `mod` holds the search path, the built-in/user merge, the sweep that reports a key the merge has left unreachable, and the active-theme cell. The engine carries no per-theme knowledge, and states the closed decoration vocabulary — a theme selects from decorations the engine already knows how to draw and states their appearance; it never describes new drawing. Parsing is also where every sprite reference is answered against its file's origin (`sprite.rs`), so a spec that has escaped it carries sources rather than strings and `Theme::resolve` stays pure. |
| `sprite.rs` | Owns turning a theme-named image into a decoration, and owns **which of the two sources it comes from**: a file beside a `themes.toml` on disk, validated against that attacker-influenced input (theme-relative, `..` refused, symlink-contained, extension allowlisted, size-capped), or bytes compiled into the binary for a **built-in** theme, which needs no file and no install step — a built-in theme is compiled in so it renders with nothing on disk, and its sprites must keep that promise. Also owns the pre-resample every GSK-drawn sprite needs at this project's GTK floor to stay crisp at zoom (linear-only texture filtering below 4.10). |
| `palette/` | Owns the resolved preview colour set and the WCAG colour maths that derives it: one `probe_named` for the desktop's named-colour chain and its floor, one `walk_to_contrast` lifting an ink to the AA floor from either polarity. Split `mod`/`tests` at the 500-line limit. |
| `cssfrag.rs` | Owns the CSS fragments the preview's sheet and the HTML sink's sheet genuinely share — the `Option<colour>` → declaration idiom, and the link-underline decision. Deliberately NOT a merged stylesheet: the two cover different surfaces through different cascades, and how a colour is SPELLED (`#rrggbbaa` on paper, `#rrggbb` on screen) stays at each call site. |
...
| `atomic_io.rs` | Owns atomic file writes (write-temp-then-rename), so a crash mid-write cannot tear a document. The sequence around the bytes — canonicalize through a symlink, probe mode and owner, create a unique private temp, arm the cleanup guard, preserve owner then relax mode, `sync_all`, close before renaming, rename, `fsync` the parent — is an `AtomicPublish` scaffold with **two** content steps: `write_atomic` writes through the handle it hands back, and an export hands the temp *path* to a library that opens the file itself. The temp is created private **before** anything writes into it, so an external writer's truncating open inherits that mode and the private-from-the-first-byte property survives. |
