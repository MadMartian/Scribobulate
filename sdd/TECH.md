...
| `clipboard.rs` | Owns what this application puts **on** a clipboard: plain text, never GTK's default rich `GtkTextBuffer`. That is what keeps a same-application paste to a SINGLE `insert-text` emission — rich content is re-inserted one chunk per syntax-highlight tag toggle, and a toggle inside a `\r\n` is the mechanism behind ScrAP-312. Covers both clipboards, which fail differently: CLIPBOARD by overriding `copy-clipboard`/`cut-clipboard`, PRIMARY by taking it over from GTK with a lazy `GdkContentProvider` subclass, since GTK republishes it on every selection change. |
| `lineendings.rs` | Owns the one rule this application holds about line separators — **a document buffer never contains a lone carriage return** — as a display-free repair applied at both doors documents arrive by: `docio`'s readers and `window::actions::load_into_editor` for files, and an `insert-text` hook for the clipboard. Never at a parse site, and `\r\n` is deliberately untouched. The hook does not make the failure impossible, it merely relies on two conditions elsewhere in the tree that currently prevent it — both are named at the function, one asserted by a test and one greppable. ScrAP-312 records why the route that WOULD make it impossible is unavailable. |
