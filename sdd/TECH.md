...
| `theme/` | Owns the reading-theme engine, in three parts. `keys` is the **registry**: the one enumeration of the vocabulary a themes file may speak, carrying each key's type, how many values it has (one, one per heading level, one per bullet depth) and whether it names a sprite — so validation, the merge, the sprite walk and the per-level fallback all read one list rather than four that can drift. `spec` is the file model: a validated map from spelling to value, plus the two-source resolution walk (a narrower spelling wins within a theme, the source wins between themes). `mod` owns the resolved `Theme` every consumer reads, the search path, and the active-theme cell. The engine carries no per-theme knowledge, and states the closed decoration vocabulary (`sdd/PLAN.preview-decoration.md`, ratified) — a theme selects from decorations the engine already knows how to draw and states their appearance; it never describes new drawing. Parsing is also where every sprite reference is answered against its file's origin (`sprite.rs`), so a spec that has escaped it carries sources rather than strings and `Theme::resolve` stays pure. |
