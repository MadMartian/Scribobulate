...
| `theme.rs` | Owns the reading-theme engine: parsing theme files and resolving keys, including the closed decoration vocabulary (`sdd/PLAN.preview-decoration.md`, ratified) — a theme selects from decorations the engine already knows how to draw and states their appearance; it never describes new drawing. Carries no per-theme knowledge. Its parse step is also where every sprite reference is answered against its file's origin (`sprite.rs`), so a spec that has escaped parsing carries sources rather than strings and `Theme::resolve` stays pure. |
