...
| `codeview/quotes.rs`, `bands.rs`, `cards.rs`, `listmarkers.rs`, `chips.rs` | One painter each: the blockquote panel and accent bar, the heading band, the code-block card, the list gutter's per-item placement, and the right-margin annotation chips. Each owns only the live geometry read and the paint; what it decides without a display is in `decorplan`. |
