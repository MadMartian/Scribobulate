...
**See**: project-specific tooling; the predicate and its corpus live in `scripts/lint-references.sh` (`win_illegal_path`) and its PowerShell twin. Content authored by the macOS seat and MEASURED there 2026-08-21 (macOS 25 Darwin.0.5, aarch64; `/usr/bin/grep --version` = "grep (BSD grep, GNU compatible) 2.6.0-FreeBSD"; `echo x | /usr/bin/grep -qP x` → exit 2, "invalid option -- P"; ugrep 7.8.4 reports `-P:pcre2jit`); landed here by the register's owning seat, edited for format and not for argument. Kin: ScrAP-318 and ScrAP-268 (a gate disarmed rather than absent), ScrAP-207 (two ports that agree by comparison instead of by derivation).
