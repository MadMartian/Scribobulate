...
**See**: kin — ScrAP-132 (a guard whose INPUT SET is not the thing it polices; this is the same defect one layer down, in what a WAIT samples), ScrAP-134 (the sibling bound-unit error), ScrAP-209, ScrAP-220.
...
**Scribobulate**: the disclosure fold splice restores the reader's offset from the viewport top after settle rather than compensating for the loss, because the number of compensating passes is not reliably countable — MEASURED bimodal at one dose (55 or 63 for the identical toggle), so the drift itself is nondeterministic. What the compensation is short BY is one `top_margin` per emission (`changed_handler`, `gtktextview.c:4918-4925`, 4.6.9: it adds `new_first_para_top - old_first_para_top` to `priv->yoffset` where the file's own invariant is `yoffset == value - top_margin`); how MANY emissions is the toggled region's own anchored-child count plus a run bounded by `gtk_text_layout_validate`'s hard-coded 2 000 px budget, and it is the REGION's children, not the view's — thirty anchored children placed outside the region cost the reader nothing. Upstream fixed it in 4.19.3 (`b300698629`). MEASURED drift, expanding: +32 px at zero region children, +368 at ten, +880 at thirty; collapsing stays inside one text row at every dose. The seven arms that established all of that measured GTK rather than this project, were bimodal and order-dependent by their own admission, and sat in a required CI gate that a distro GTK patch could redden with nothing here broken — so they were **deleted** once this entry and the skill entry below carried their conclusion. Read them in `git log -- src/preview/splice/excursion/` rather than re-deriving them. What remains under `preview::splice::excursion` asserts this project's own contract: the excursion the splice avoids, and (in `excursion::wired`) the reader landing on their own line through the production path.
