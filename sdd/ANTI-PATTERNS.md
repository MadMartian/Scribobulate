...
**Open, and deliberately not claimed either way** (raised by the skill maintainer, out with the researcher 2026-08-16): the damage above is a *path*-watching failure, yet the one platform where the original cause bites (HFS+/NFD) is the one whose monitor watches the **inode** (kqueue, ScrAP-269). So either the worst cases never overlap, or kqueue fails earlier and *louder* — it must `open()` something for its fd, so a non-existent path plausibly errors at attach instead of watching nothing. The question that settles it is one question: **what does `g_file_monitor_file()` do with a path that does not exist, per backend?** If Linux/inotify silently degrades to `GPollFileMonitor`, this whole class is masked here and is a macOS/Windows symptom.
