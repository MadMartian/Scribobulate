...
**Scribobulate**: `src/lineendings.rs` owns the rule and the display-free repair; `docio`'s three readers and `window::actions::load_into_editor` apply it, and that half is landed and sound. Guards: the three-reader and LF-twin-parse tests in `docio`, the undo-stack test in `window::actions`, the display-free set in `lineendings`, and `a_same_application_rich_paste_keeps_crlf_intact_across_run_boundaries` — a **standing guard on the ABSENCE** of a buffer-mutating `insert-text` hook, which asserts the multi-run condition as a precondition so a tag geometry that happens not to split cannot green it vacuously. **The clipboard half landed separately and in the right order**: first `crate::clipboard` stopped publishing a `GtkTextBuffer`, which makes every paste arrive in a fresh untagged buffer as ONE emission and removes the chunking rather than the one boundary we knew about; only then did the repair return, as `lineendings::buffer::ScribBuffer`'s `insert_text` **vfunc** — correct by construction where the signal is not, since gtk4-rs's `connect_insert_text` hands the closure a copy of the caller's iterator and never writes it back. Guard: `a_same_application_paste_arrives_as_a_single_emission` (mutation-checked: 6 emissions with the override removed, 1 with it). Still open, and filed rather than decided, is the undo-bracket question: bracketing the replay preserves the original bytes and violates the invariant, not bracketing means undo lies about what it undid. `probes/` carries the rigs for every measurement above. TDD §1.10, `tests/MANUAL-TEST.md` check 1.10. `\r\n` deliberately untouched (MANUAL-TEST §4.2).
