...
| 311 | Which of two same-key bindings wins is a property of the BACKEND, not of the toolkit — Quartz is the minority behaviour, and stating it neutrally names the exception as the rule | C |
| 312 | Repairing the editor buffer when the defect is in the source string — the preview is built from the source and never reads the buffer; plus the sequel, a content guard that cannot see a lone CR the CHUNKING manufactured | C |
| 313 | `GtkTextBufferContent` refs the buffer and never unrefs it, so a whole document leaks on every select-then-deselect — and it contaminates any leak test written near it | A |
| 314 | Instantiating ANY `sourceview::Buffer` subclass corrupts the heap; the crash lands in `g_slice_alloc` with an innocent backtrace, and four theories died before one survived | C |
...
| 319 | A portability gate's verdict depends on which `grep` is first on PATH — and the seat that should catch the bug is the seat whose interactive shell hides it | B |
| 320 | The unresolvable pointer — sending a reference the recipient cannot dereference, across a filesystem, seat, TTL or gitignore boundary, and mistaking the send for a delivery | B |
...
| 336 | A `GtkPaned` child dragged to nothing still reports its full natural `height()` — GTK parks it off-canvas rather than shrinking it, so the obvious oracle cannot see the crush | A |
...
**Citing the halves**: `34a`/`34b` are sub-labels INSIDE this entry, not entry IDs — there is no `## 34a.` heading and there must not be one, because the number manifest holds headings and check 9 fails on an allocated number with no heading (measured). Cite `ScrAP-34` and name the half in words. This matters because `ScrAP-23a` DOES name a whole entry, so the suffixed spelling would otherwise mean two different things with no way for a reader to tell — check 8's own argument against a bare `AP-N`.
...
**Upstream filing**: researcher-verified as a genuine GTK defect with a ready-to-file GitLab issue drafted (summary, C reproducer, root cause, one-line suggested fix). Filing was recommended and never done. The draft lived for a while stapled to an UNRELATED `sdd/ISSUES.md` entry, where its pasted `## Summary`/`## Root cause` headings sat at entry-heading level and read as phantom entries; it was removed from there rather than moved, and is recoverable from git at `16fdc45`. Keep the workaround above regardless — an upstream patch would not backport to 4.6/4.8.
...
**Symptom**: a leak fix that uniformly converted every dialog's self-closure from strong `.clone()` to weak `.downgrade()` broke every `FileChooserNative` dialog outright (failed to appear/respond); and, on macOS, tearing a native dialog down while it is still VISIBLE silently orphans GTK's whole Quartz backend struct — 12 live `GtkFileChooserNative`/`GtkFileChooserDialog`/`GtkFileChooserWidget` and 1632 `GtkColumnViewCell` after 12 cycles — where the identical call after `response` leaves every count at zero.
**Scribobulate**: split by type — `gtk::Window` dialogs keep a weak self-ref (toplevel list pins them) while any `NativeDialog` keeps ONE strong ref in an external `Rc<RefCell<Option<…>>>` holder (`saferizer::native_dialog::NativeDialogHolder`), dropped in `connect_response` after `.destroy()`; that ordering is also what keeps both call sites (`window/export.rs`'s `choose_destination`, `app/appactions.rs`'s open action) off the visible-teardown path, so **never call `hide()`/`destroy()` on a native dialog that is still showing, and never cancel a `GCancellable` a dialog is waiting on** — destroy from the `response` handler, which is structurally safe because `_gtk_native_dialog_emit_response` clears `visible` first.
**See**: gtk4-rs skill → threading-async-and-memory (GTK4Rs/AP-41, and GTK4Rs/AP-304 for the Quartz ownership defect that makes the visible-teardown path costly — unfixed upstream in every GTK 4.x, inherited by `GtkFileDialog`).
...
**Resolution**: a dedicated tab-normalization step replaces a hard tab with ONE space — LENGTH- and POSITION-preserving (so copymap/scroll-sync byte offsets never drift) — exempting leading indentation and verbatim code regions (found via a structural pre-parse). It runs at **all four** parse sites (`preview/build.rs`, `export/doc.rs`, `outline.rs`, `copymap::balance_source_span`), enumerated and test-backed rather than summarised; it reached two of them for a while, and both uncovered surfaces produced wrong results (a phantom outline heading; an annotation spanning a table cell boundary). **Now enforced by `renderer::NormalizedMd`** (encapsulation): its constructor is the only route to a normalised document string, and the pre-pass function it wraps is module-private, so a parse site can no longer call it directly and can no longer forget to call it while still going through the seam. A `clippy.toml` ban on the raw `pulldown_cmark::Parser::new_ext` constructor was assessed and rejected — every existing caller of it is a legitimate non-seam use (the pre-pass's own structural scan of un-normalised text, three sites parsing CriticMarkup-cleaned text derived from the normalised text, and several tests exercising the tokeniser directly), so the ban would fire on legitimate calls, not bypasses.
...
**Symptom**: a guard is green and the property it asserts is false — because it never looked at the live thing. Two mechanisms, one root cause: a config-dir scanner passed with a deliberate violation injected (it was scanning an empty string), and the menu access-key uniqueness guard passed over a live `l` collision (`Save A_ll` vs `_Load Unsafe Linked Documents`) because its popover lists were a hand-maintained mirror of the menu that had fallen eight entries behind. The assertion was right in both; the input set was not.
**Scribobulate**: `app::mnemonics::menu_access_keys_unique_per_popover` now DERIVES its popover grouping from `app::menubar::build_top_level_menus` — the same models `build_menubar` ships — instead of mirroring them, and pins its own non-vacuity plus the dynamic-popover exemptions; deriving found the collision on its first run and two commands that had shipped with no access key at all. The general corrective is the same at both sites: make the guard READ the artefact, and where a mirror is unavoidable, mutation-test that the walk actually reaches the items it claims to. This entry also describes the now-retired config-dir-resolution scanner (the "no module resolves the config dir through `glib::user_config_dir()`" guard) — its `#[cfg(test)]`-truncation bug, and later its hardcoded-4-file scope (itself this very species), meant it never scanned the sites it policed. That invariant is now enforced compiler-wide by a `clippy.toml` `disallowed-methods` ban on `glib::user_config_dir` (N10) — a gate with no scope to get wrong. The one surviving injection-verified absence-guard is `workaround.rs::the_redirect_touches_config_home_only` (the redirect's `set_var(` lines must name `XDG_CONFIG_HOME` and never the data dirs), whose single-file scope is correct because `set_var` only happens there.
...
**⚠️ THE GUARD IS INERT ON GTK 4.22.4 / Quartz — a green macOS suite proves nothing about this
fix.** MEASURED by mutation on the macOS seat: neutering the re-anchor
(`scrollpos::jump(&…outline_scroller.vadjustment(), 0.0)` in `outline_collapse_all`) leaves
`collapse_all_leaves_only_the_root_row` **passing**, and no other test in the suite catches the
removal either — so that line has **zero** coverage on that platform, not merely weak coverage.
**It is not a broken or vacuous test**, and the distinction decides whether anything needs
fixing: the guard's own precondition assertions pass, the deep row materialises, and the
collapse runs from the scrolled-to-bottom state. `GtkListView` 4.22.4 simply **recovers** from
losing its scroll anchor to a removal, where 4.6 stranded the far-end leaf. The guard is
well-built and fires correctly on the platform the defect belongs to.
**The realistic failure this warns about is the inverse of the usual one**: not a missed
regression, but someone reading the re-anchor as dead code and deleting it after a green macOS
run — with only Linux 4.6.9 catching it. That is POLICY's *Linux is the gate* rule working
exactly as designed, and this is a measured instance of it rather than a new hazard.
...
**Scribobulate**: `scripts/lint-references.scan` — one enumeration definition the gate reads rather than restates, with `maxdepth` as a hard tripwire rather than a filter. (The two ports that had drifted are themselves retired: the gate is one `cargo xtask` binary, so the enumeration can no longer be two.) **SECOND INSTANCE, one level out — the ports agreed on the LIST and differed on whether they could EXECUTE it.** `scripts/pipeline.steps`' grammar allows a leading `NAME=VALUE ` env prefix on a `cmd.<plat>` line; `pipeline.sh` gets it free from `eval`, and the PowerShell port — which dispatches through `cmd /c`, a shell with no such syntax — never implemented it. The first line to use one failed with *"'G_DEBUG' is not recognized"*, so the step that exists to turn a `Gtk-CRITICAL` into a hard failure produced a hard failure no critical caused; **a gate that cannot RUN is worse than one that is absent, because its red reads as a verdict.** And every parity check was green throughout — `--list-steps`, `--self-test` and the contract mutations all pass without EXECUTING a single step, so both ports could be incapable of running a command and nothing would say so. The self-tests validate DERIVATION, never EXECUTION; adding a grammar feature therefore obliges you to run it on every port. **And discharge that obligation by mutating THROUGH the runner, not around it** — a green pipeline under a newly-implemented env prefix is indistinguishable from one where the parser set nothing, so the check that settles it is to plant a defect the variable is supposed to catch and watch the real runner go red (measured: re-applying a `GTK_IS_WIDGET` critical failed step 5 with `0xC0000409`, proving the variable reached the cargo child rather than merely that the parser parsed).
**Kin, same shape one level along — EVIDENCE STRUCTURALLY INCAPABLE OF DISCRIMINATING.** This entry is about a check that proved *agreement* without proving *conformance*. The sibling case is a CHANGE verified against inputs that cannot reach it: the two bash runners' step-disposition branch order diverged from the PowerShell port's, and the divergence was unreachable from the contract — no step is both `packaging` and `na.` on any platform — so no run of the real pipeline could distinguish the two orders and a green suite after a reorder would have been evidence of nothing. The corrective is the same in both directions: **when the behaviour under change cannot be reached by any real input, extract it until it can be, rather than changing it and treating the untouched green suite as evidence.** Done here by lifting the decision into a pure `step_disposition` and pinning it with a table whose load-bearing row is precisely the unreachable case (`scripts/pipeline-lib.sh`). Deliberately NOT minted as its own number: the general form is verification method rather than a fact about this tree, and it cost nothing — the wrong path was identified before it was taken.
...
**Scribobulate**: `cargo xtask lint-references` check 8 (the per-site migration rule and the audit's limits are documented at the check, next to the gate that enforces the form); the citation-convention paragraphs at the top of this file and at "Numbering reconciliation"; POLICY step 9. Retires the citations-registration plan (its analysis lives on in git history).
**See**: general-engineering-principles (GEP-24).
...
**Symptom**: a rename regression guard stays green with the fix removed (two independently sufficient cancels mask each other), and separately a document rename is announced to the user as *"your file was deleted"* — on macOS especially, where the backend emits `DELETED` twice and a guard that consumes exactly one event lets the second through.
**Scribobulate**: `window/rename.rs` cancels the monitor before the rename and re-attaches on **every** path including failure; `the_old_monitor_is_cancelled_before_the_rename_touches_the_filesystem` is the state guard (mutation-checked: removing the cancel fails it and only it), and `a_rename_does_not_look_like_a_deletion` keeps the behavioural contract with its non-optional in-test control (delete the renamed file, assert the notice *does* appear — without it the test passes against a dead monitor).
**See**: gtk4-rs skill → app-lifecycle-and-env (GTK4Rs/AP-269), which carries the GIO source trace, the per-backend event sets and counts, and the pre-2.84 `cancelled`-property defect. The full essay is in this file's git history.
...
> *Authored by the Windows seat from its own measurement of 00df21e; the Linux leg measured here; allocated and formatted by this seat, its reasoning and labels unchanged. Kin GTK4Rs/AP-121.*

**Symptom**: a fix, a TDD rubric and a module doc all state as settled fact that `GtkSourceView`'s `move-words` class keybinding beats a window `GAction` accelerator declared on the same keystroke while the view holds focus. On Quartz that is measured and true. On Win32 and on X11, with the same key, a `GtkSourceView` that demonstrably carries the binding, the **accelerator wins and `move-words` never fires**.
**Measured, three backends, one keystroke** (Alt/Option+Left, editor focused, history non-empty):
- **Quartz**, GTK 4.22.4 — `move-words` fires, the line is transposed, `win.nav-back` never runs. (macOS seat, 00df21e.)
- **Win32**, GTK 4.22.4 / GtkSourceView 5.20.0 — `win.nav-back` runs (tab switches, caret restored, Forward enables), buffer byte-identical, files unchanged by sha256. `move-words` present in `gtksourceview-5-0.dll` with `move-lines`/`change-case`/`show-completion` as positive controls and a bogus needle correctly absent, so it exists and **loses**. (Windows seat.)
- **X11**, GTK 4.6.9 / GtkSourceView 5.4.1 — same as Win32: title `alpha.md` → `beta.md`, `nav-forward` enabled, buffer unchanged through a save, `Alt+Right` retraces. `move-words` present in `libgtksourceview-5.so` with the same controls. Delivery proven first by typing a character and reading it back off disk. (This seat, Xvfb + kwin, release binary at 00df21e.)
...
**Resolution**: **state the winner of a keybinding contest together with the backend it was measured on.** A sentence like *"the class binding wins that contest"* reads as a toolkit property and will be inherited as one; the next seat then either ports a fix nobody needs or skips one somebody does. Quartz is 1 of 3 here, so the platform-neutral spelling was not merely unscoped, it named the minority behaviour as the rule.
**Two riders from the Windows run, both INFERRED**: on Win32 the accelerator appears to **consume** Alt+Left even when its action is *disabled* (empty history: neither navigation nor transposition, and `move-words` did not fire either) — which is not what GTK4Rs/AP-121's entry-field case does, where a failed activation lets propagation continue. Nobody instrumented the handler.
**Harness traps, and the reusable half** (why two of the three runs discriminated nothing): with **history empty**, Alt+Left produces no pixel change at all and a disabled `GSimpleAction` activates as a silent no-op — *"nothing happened"* is satisfied by **both** hypotheses. With **focus on the tab strip**, the class binding is never in contention, so the run "confirms" the accelerator won a contest that never took place. Only both conditions held at once is decisive. Two controls cost a cycle each: a delivery control keyed on a **window-title dirty marker this application does not have** (it is in the status bar and the tab dot) reads exactly like a control that failed; and `Ctrl+End` lands on the empty line after the trailing newline, where `move-words` discriminates nothing — use `^{END}{UP}{END}`.
...
> *Non-core (pulldown-cmark/CommonMark plus this project's own ingress topology) — stays here per the routing rule. Symptom, mechanism and corrective authored by the macOS seat from its live reproduction; platform scope, the CRLF run-boundary control and the three-arm isolation measured by this seat; the GTK and gtk4-rs mechanics are routed to the `gtk4-rs` skill and deliberately not restated here.*

**Symptom**: a document whose lines are separated by a bare `\r` renders in the preview as **one enormous heading** containing the whole file, and in the outline as one entry — while the **editor pane beside it shows the lines correctly** and the footer reports the right `Ln`/`Col`. The split between the two panes is the tell; checking either alone misses it.
**Root cause**: GTK and pulldown-cmark disagree totally about whether a bare `\r` is a line ending. MEASURED on both platforms — Quartz/4.22.4 and X11/4.6.9: the buffer reports `line_count = 6` and highlights the heading, while the byte-identical string parses to one `Start(Heading(H1))` swallowing the document. **Not platform-specific in any part**, so the "Mac bug" framing was wrong — as was "Mac uses CR": macOS has used `\n` since Mac OS X, and the CR comes from a *sharing tool's* clipboard bridge converting to a convention abandoned in 2001.
**What was tried, and why it read as success**: normalising in `window::actions::load_into_editor`, where text enters the `GtkTextBuffer`. One layer too late — `assemble_tab_core` hands the **same source string** to `build_tab_editor` *and* to the preview builder, and `preview/build.rs` parses that string. The repair fixed the pane that already looked right. 1007 unit tests and the whole GTK suite passed on that version.
**What discriminated**: rendering the byte-identical **LF twin** in the same build and comparing screenshots. Nothing else did. Near-miss inside that check: the first "after" screenshot was of a **stale installed build**, because a bare launch forwarded the document to the running primary via the single-instance handoff — launch with `-n`, and confirm the pid you screenshot is the pid you launched.
**Resolution**: repair at the **ingress doors**, never at a parse site. (a) `docio`'s readers, beside `without_bom`, at **all three** — the save guard and crash recovery each compare on-disk content against an in-memory baseline, so repairing one side of a comparison turns every save into a spurious "changed on disk" prompt. (b) the buffer's `insert-text`, for clipboard/DnD/PRIMARY. **Not** the parse pre-pass: `renderer::normalize_inline_tabs` runs when a document is PARSED, which is a different moment from when text arrives — it cannot reach the save guard, crash recovery, or the editor buffer at all. (Its own reach was separately a defect: it claimed "every parse site" while reaching two of four. Now four of four, with the enumeration test-backed — see the entry below on unbacked contract claims.)
**Why the clipboard half was REJECTED — two independent defects in one handler**. (1) **CRLF corruption.** The unreachability proof rested on an invariant over the payload's CONTENT (no buffer holds a lone `\r`) and silently treated it as one over the payload's DELIMITATION. `insert_range_not_inside_self` chunks a same-app copy on tag toggles with no line-terminator awareness, and a toggle CAN land inside a CRLF — so the chunking **manufactures** a lone `\r` that no buffer contains, the guard fires, and `\r\n` becomes `\n\n`. MEASURED both backends, no version drift, trigger = a `no-spell-check` context still open at EOF (an unclosed fence); close the fence and it is byte-identical. It needs no explicit copy: PRIMARY carries the tagged buffer, so SELECTING and middle-clicking is enough. **This half is diagnostic-silent.** (2) **A stale destination iterator**, whose cause is the gtk4-rs binding rather than anything we wrote — on a longer multi-run paste the document comes back **SCRAMBLED**, not merely re-terminated. Tree audit: `grep -rn connect_insert_text src/` returns two hits, both in `lineendings.rs`, so exposure is bounded to this change; the `TextBufferImpl::insert_text` vfunc is the correct shape and anything new reaching for the signal inherits the defect. **Adopt the vfunc with one stated limit, because this is the kind of caveat that gets dropped between the finding and the implementation**: gtk4-rs's subclass trampoline ignores its `_length` and reads the pointer as NUL-terminated, so a **bounded-length** caller gets the wrong text — CONFIRMED in the binding we ship against, `insert_markup("AB<b>CD</b>EF")` yielding `"ABCDEFCDEFEF"`, every segment re-read to the end of the underlying string. Neither a truncation nor a crash: duplicated trailing text at each attribute boundary, which reads as "my document grew garbage". **The exposure condition is INBOUND, which is easy to get backwards**: a repair handler rebuilding a string cannot reach it, because `parent_insert_text` calls the C parent's function pointer directly and a fresh emission from Rust goes through `TextBufferExt::insert` where `len == strlen` by construction. It needs GTK itself to pass a bounded length. So: adopt it, and do NOT let `insert_markup`, `insert_with_attributes`, `GtkSourceCompletionWords` or vim mode reach it until the binding is fixed upstream. Not live here (`grep -rniE "snippet|completion|insert_markup|insert_with_attributes|insert_with_tags" src/` returns only unrelated prose, and no `GtkSourceCompletion` or snippet bundle is instantiated). **Now established, and the trigger is narrower than "completion"**: GtkSourceView 5.20.0 reaches a bounded-length insert at exactly two sites — `GtkSourceCompletionWords` (accepting a proposal mid-word) and vim-mode command handling. Neither is instantiated here. Its file-LOADING path is safe because GtkSourceView temporarily NUL-terminates there on purpose, a workaround it has carried since 2014 for **this same defect in PyGObject** (`gtksourcebufferoutputstream.c`, citing GNOME bug 726689) — so `docio`'s route is defended by the library, not by us. The guard to write down is therefore specific: **re-run the binding-shape probe's markup arm before enabling `GtkSourceCompletionWords` or vim mode.** Another completion provider would need its own check rather than inheriting this one. Mechanics, the one-site audit that scopes it, and the vfunc's own counted-string caveat live in the `gtk4-rs` skill.
**Do not lead a reader with the diagnostics.** They belong to defect (2); defect (1) raises nothing, anywhere. Chase them and you fix the lesser bug, watch them vanish, and ship the corruption.
**`G_DEBUG=fatal-criticals` does not catch defect (1) on any platform.** It guards against MISUSE OF THE API and says nothing about WRONG ANSWERS FROM CORRECT USE. The test that caught it asserts on `\r`/`\n` occurrence counts; nothing runtime fired.
**Scribobulate**: `src/lineendings.rs` owns the rule and the display-free repair; `docio`'s three readers and `window::actions::load_into_editor` apply it, and that half is landed and sound. Guards: the three-reader and LF-twin-parse tests in `docio`, the undo-stack test in `window::actions`, and the display-free set in `lineendings`. **The clipboard half landed separately and in the right order**: first `crate::clipboard` stopped publishing a `GtkTextBuffer`, which makes every paste arrive in a fresh untagged buffer as ONE emission and removes the chunking rather than the one boundary we knew about; only then did the repair return. It returned as the **signal**, `lineendings::wire_paste_normalization`, and NOT as the `insert_text` vfunc that would be correct by construction — that route is blocked by #314, so the shipped hook is correct by INVARIANT and both invariants are named at the function. Guard: `a_same_application_paste_arrives_as_a_single_emission` (mutation-checked: 6 emissions with the override removed, 1 with it). The undo-bracket question this entry left open is **decided** in #316, which also removes the ordering the decision rested on. `probes/` carries the rigs for every measurement above. TDD §1.10, `tests/MANUAL-TEST.md` check 1.10. `\r\n` deliberately untouched (MANUAL-TEST §4.2).
**Lessons**, five, and the first four are the same shape — *check the thing that PRODUCES the thing in front of you*. (1) **When two views of a document disagree, fix the input they SHARE**, not the one whose output is wrong; a green suite over a fix sited one layer too late is the expected outcome. (2) **A guard on a payload's content cannot see a condition its DELIMITATION manufactures** — an invariant over the whole is not an invariant over any chunk a framework chose, and a two-byte sequence is exactly what a boundary can cut in half. (3) **A cross-platform difference observed through two different rigs is a RIG difference until proven otherwise**; the "silent on Quartz" reading was a binding difference, and only flagging the confound rather than the finding saved it. (4) **Before predicting where a defect is reachable, NAME THE CALLER.** Lessons 1-3 and the inbound/outbound slip on the length defect are all one habit — reasoning about which side of an interface a value crosses without checking which side actually invokes the code. Naming the caller is the cheap question that would have caught every one. (5) Method, and the sharpest form of it anyone here reached: **a theory that explains the observation is not evidence; a control that should break and does not, is.** Every theory this bug killed — four of them, held by two seats and the researcher — died to a control that changed ONE variable, never to rethinking. Corollary: **"we reproduced it" and "we explained it" are separate claims** — a three-arm isolation established the first here and two seats then explained the second wrong, in turn. Corollaries with teeth: a regression test for a boundary condition must ASSERT the boundary was exercised, or it reports on a case it never reached — and that assertion catches a broken *rig* too, which is how a probe with mismatched tag tables was caught reading as a clean null. And a pre-pass that *claims* to cover "every call site" is a claim, not a gate: count the callers.

## 313. `GtkTextBufferContent` refs the buffer and never unrefs it — a whole document leaks per select-then-deselect
> *Core GTK — a weave candidate for the `gtk4-rs` skill, not yet routed because it is unreported upstream and the fix is not ours. Found by the researcher while building the PRIMARY provider for ScrAP-312.*

**Symptom**: memory grows in proportion to how often the reader selects text, and a closed tab's document is never freed. Nothing warns, no application code appears in any leak trace, and it happens whether or not the application touches a clipboard at all.
**Root cause**: with a selection, `update_selection_clipboards` creates `selection_content = gtk_text_buffer_content_new (buffer)`, which takes `g_object_ref (buffer)` — a self-cycle. On deselect it sets that content's `text_buffer = NULL` **without unreffing**, then `g_clear_object`s the content. `GtkTextBufferContent` implements no `finalize` or `dispose`, so the reference is dropped on the floor. MEASURED, plain C, refcount of one buffer: fresh `1`, after `select_range` `2`, after deselect **still `2`**, and `+1` per further cycle — `8` after seven. Source-read byte-identical at 4.6.9, 4.12.5 and 4.22.4.
**Not avoidable from application code**: `update_selection_clipboards` builds `selection_content` whether or not any selection clipboard is registered, so no application change makes it better or worse. Taking PRIMARY over with a custom provider does not fix it either — it only avoids *adding* a second strong reference. (This project once held such a provider with a `WeakRef` for that reason; it no longer has one at all — see the Scribobulate line below.)
**The trap this entry exists for, and it costs a day**: it **contaminates any leak test written near a text buffer**. A "does closing a tab free the document?" check fails for this reason and reads as a regression caused by whatever change is in flight. **Write the control first** — the same check with no selection ever made — before concluding your change leaked anything.
**Scribobulate**: none — a limitation with no implementation in this tree. (Stated, not omitted: an absent field and a dropped one look identical.) **Corrected 2026-08-22**: this line used to say the PRIMARY provider in `clipboard.rs` holds a `WeakRef` for the adjacent reason. There is no PRIMARY provider anywhere in this tree any more, on either pane — `wire_primary_selection` and `primarysel.rs` were deleted in `4b97c84` when the middle-click paste was fixed on the CONSUMER side instead. The leak is unaffected by that removal, exactly as the paragraph above says: `update_selection_clipboards` builds `selection_content` whether or not any provider is registered, so it is GTK's own book-keeping either way.
**See**: project-specific until routed; the mechanism, the refcount table and the negative tracker search live here. A GNOME tracker search for `GtkTextBufferContent`, `GtkTextBuffer leak` and `text buffer leak selection` found nothing matching — a negative search result, not proof it is unreported. The refcount table above is the whole upstream report, whenever someone files it.

## 314. Instantiating ANY `sourceview::Buffer` subclass corrupts the heap — and the backtrace is innocent
> *Non-core (GtkSourceView + gtk4-rs subclassing) — stays here per the routing rule. UNRESOLVED: the mechanism is not established, and this entry exists to stop the next person re-deriving four dead theories.*
...
**Read the backtrace as a victim, not a witness.** A crash inside `g_slice_alloc` is heap corruption tripping over a free list some earlier write damaged, so every frame in it is innocent. Chasing it costs a day and finds nothing.
**Isolated to one sentence — instantiating the subclass, not using it**:
```
A  no subclass at all                            1256 passed, clean
B  subclass + insert_text override + used        SIGSEGV
C  subclass + override, not used in production   SIGSEGV  (still instantiated in a test)
D  subclass DEFINED, never INSTANTIATED          1256 passed
E  subclass INSTANTIATED, override EMPTY         SIGSEGV
```
D and E are the load-bearing arms. D is clean because `static_type()` registers the GType but does **not** run `class_init` — that happens on the first `g_type_class_ref`, which construction triggers. E crashes with an empty `impl TextBufferImpl {}`, which is *not* "no override" but "override that chains up", so it still installs the trampolines.
**FOUR THEORIES ELIMINATED, all by measurement — do not re-derive them.** (1) *Our repair body* — E crashes with an empty impl. (2) *gtk4-rs's `insert_text` trampoline ignoring its `_length`* — that defect is real (it inserts the wrong text given a bounded length) but a subclass with an empty impl AND one with a real repairing body both survived **400 rounds** in isolation on the same pool thread, with markdown highlighting, a live view and undo/redo replay, clean under valgrind with `G_SLICE=always-malloc G_DEBUG=gc-friendly`: zero invalid reads or writes. (3) *A bounded-length caller reaching that trampoline* — there is none in this process: no `insert_markup`, `insert_with_attributes`, `insert_with_tags`, completion or vim; GtkSourceView's file loader NUL-terminates deliberately; and the undo manager's `IString` is always NUL-terminated at `n_bytes`. (4) *A panic crossing the FFI boundary* — that aborts (SIGABRT) since Rust 1.81, and this is SIGSEGV. A duplicate GType name is also out: glib panics loudly.
**Surviving hypothesis, UNCONFIRMED and to be distrusted precisely because it fits everything**: an instance created on `test_synced`'s single GTK thread being refcounted, mutated or finalised from one of libtest's plain-`#[test]` worker threads. It explains every arm — including why only a full `--lib` run mixes the two worlds — which is exactly the property that makes a theory cheap. **The falsifier is one line**: assert `std::thread::current().id()` matches between `ObjectImpl::constructed` and `dispose`. If they match, strike the hypothesis rather than carrying it. Also unrun: `--test-threads=1` on the full `--lib`.
**Scribobulate**: it forecloses the mechanism that would have made ScrAP-312's clipboard repair impossible-to-break, so that repair ships as a marked fence instead — the two conditions it depends on are named at `lineendings::wire_paste_normalization`. Nothing else in this tree subclasses a GtkSourceView type; five other `glib::wrapper!` subclasses of plain GTK types are unaffected.
**Lesson**: **when a crash lands in an allocator, the bug is somewhere you have already been.** Stop reading the stack and start deleting variables — and prefer an arm that makes the suspect *exist but do nothing* (E) over one that removes it entirely (A), because only the first distinguishes "this code is wrong" from "this thing existing is wrong".

## 315. Laying a table out with tab characters — a tab ladder cannot express a column
> *Non-core (Pango + this project's own export sink) — stays here in full per the routing rule; do NOT fold into the gtk4-rs skill. Authored by the seat that fixed it; allocated and formatted here, its reasoning unchanged, including a correction that seat sent after this entry was first drafted.*
...
**Root cause**: `export/pdf.rs`'s `table()` joined each row's cells with `\t` and emitted one ordinary Pango paragraph per row. A tab advances to the next stop in a **fixed ladder** (24pt here), so a cell one character too wide shoves its neighbour a whole stop right. MEASURED on the reporter's own artefact with `pdftotext -bbox-layout`: the Date/Time column began at x=120 on one row, 144 on the next, 216 on the last. The delimiter row's column alignments were parsed and then thrown away (`let _ = aligns;`).
...
**Scribobulate**: `src/export/pdftable.rs` (99.5% covered) + `export/pdf.rs`'s table path. Guards: `export::pdftable::tests`, `export::pdf::pdf_layout_tests`' table set — chiefly `every_row_of_a_table_shares_one_column_grid` — and `the_shared_rule_agrees_with_the_previews_own_fit_columns`, which runs identical inputs through both implementations. `tests/MANUAL-TEST.md` 25.17a asserts one `xMin` per column off the real artefact.
**Lesson 1 — the input side already knew.** ScrAP-75 records that a hard TAB inside a GFM table breaks table recognition, and the fix normalises tabs away on the way IN. The export sink then chose tabs as its column mechanism on the way OUT. Same character, same wrong job, opposite direction, and nothing connected them because one is a parser concern and the other a layout concern.
**Lesson 2 — a doc comment asserted a behaviour the code had never implemented, and THAT is what kept the gap invisible.** `table()`'s rustdoc said *"Scale-to-fit is applied by the drawing pass from the width this measured (TDD 25.17)"*. No scale-to-fit existed anywhere. The same comment claimed each row was an indivisible fragment so a page break could never fall through one, while `paragraph()` set a wrap width, so an over-wide row split into independently paginable pieces (MEASURED, a row splitting as `…FR\tSingle ` / `method`). The file **documented conformance to TDD 25.17 and 25.16 while satisfying neither**, and a reader auditing the rubric against the code would have found the citation and stopped. TDD 25.17 was manual-only and never automated, which is how it shipped unmet. **A comment claiming a contract is satisfied is a claim that needs a TEST behind it, not prose** — an unbacked rubric citation is worse than no comment, because it terminates the audit it appears to serve. (The same class was open against `renderer::normalize_inline_tabs`, which claimed "every parse site" while reaching two of four; MEASURED and closed — the outline listed a phantom H1 whose text was an entire tab-padded table, and `copymap::balance_source_span` swallowed an emphasis run across a table cell boundary. Both uncovered sites now call the pre-pass, and the enumeration of the four is held by `renderer::normalize::normalize_inline_tabs_tests::every_parse_site_reads_one_document` rather than by the sentence — which is the corrective this lesson prescribes, applied to itself.)
**Lesson 3 — a justification that is TRUE is not thereby a justification for the thing in front of you.** The first version of this fix used a proportional-share width rule and defended the divergence from the preview's water-fill with *"a pane can scroll and paper cannot"*. That statement is true. It bears **only** on what happens when nothing fits — and it was used to license a different sharing rule in the regimes where everything DOES fit, which scrolling says nothing about. Worse, `widgets::table::layout::fit_columns`'s own doc comment rejects that scheme in those words (*"instead of a proportional-to-want scheme that would give nearly all space to the widest column"*), so the warning had been read and the thing it warns against implemented anyway. The rule is now water-fill, shared with the preview; only the overflow endgame differs, which is the one regime the argument actually reached. **When a real difference between two contexts is used to license a difference in behaviour, check that the licensed difference is the SAME one the argument establishes.** It survived both its author's review and this seat's ratification because both were checking whether the argument was *true* rather than whether it *reached*.
**The shared shape of lessons 2 and 3, which is the transferable core**: a citation standing in for a check nobody then performs. One was a rubric reference that ended an audit; the other a design argument that ended a design question. Both looked like evidence and were pointers.

## 316. A repair handler on `insert-text` is also a handler on the UNDO machinery, and the divergence it causes is silent
> *Split: the GTK mechanic (`GtkTextHistory` replaying through the public insert, and its ignored `expected_text`) is core-GTK and is routed to the `gtk4-rs` skill maintainer; the remedy trade and the construction that settles it are this project's and stay here. Measured with `probes/undo-replay.c` before this entry existed; the decision taken and enforced here.*
...
**Root cause**: `gtk_text_buffer_history_insert` reaches the buffer through the **public** `gtk_text_buffer_insert`, so a handler that rewrites inserted text is on the replay path as much as on the paste path — it was never opted out of. `GtkTextHistory` cannot notice: it sets `applying` for the whole replay and suppresses its own bookkeeping, and the `expected_text` its delete path is handed is ignored by GTK's implementation, so the one value that could have detected the substitution is never compared. MEASURED on GTK 4.22.4 / GtkSourceView 5.20.0 and matching 4.6.9 byte for byte; `probes/undo-replay.c` is the rig.
**Both remedies cost something, which is why the choice had to be made explicitly rather than fallen into**: bracket the replay (a `connect` plus `connect_after` pair on `undo`/`redo` straddles the default handler exactly) and the original bytes go back — including a lone `\r`, re-entering a buffer through undo, where the preview collapses the whole document into one heading and the outline into one entry. Do not bracket, and undo silently restores something other than what it undid. There is no third option **at the replay**.
**Resolution — take the choice off the replay entirely.** Do not bracket: the no-lone-CR invariant outranks byte-exact undo of a sequence no buffer may legally hold, and the two only ever differ on a buffer that already violates the invariant. Then make that buffer impossible: `lineendings::new_editor_buffer` creates and arms in one call, and `wire_paste_normalization` is private to that module, so no caller can populate a buffer and arm the repair afterwards. The repair is consequently a no-op on every replay this process can reach, and the trade is settled by construction rather than re-litigated at each call site.
**Lesson 1 — "unreachable today" is a property of an ORDERING, and an ordering is one line away from being different.** This was filed as not-currently-reachable on the strength of two lines appearing in the right order inside one function, with a comment saying the order was load-bearing. That comment is the whole defence, it is invisible to every gate, and the paths that would break it are ordinary ones nobody would suspect — session restore, template insertion, duplicating a tab. **When a defect's unreachability rests on an ordering, collapse the two steps into one call and make the wrong order non-expressible**; a load-bearing comment is a bug with a note attached.
**Lesson 2 — enumerate a signal's CALLERS, not its purpose.** `insert-text` reads as "the paste hook" because that is what it was wired for, and every review of it asked what a paste could carry. The undo machinery calls the same public entry point; so do `set_text`, `insert_at_cursor` and every internal convenience. A handler's blast radius is the set of callers of the function it hangs off, which is knowable, and is never the set the author had in mind. (Same habit as ScrAP-312's fourth lesson, one interface further in — that one named the caller to scope a *defect*, this one to scope a *handler*.)
**Lesson 3 — a framework that suppresses its own bookkeeping during a replay suppresses your evidence too.** `applying` is why nothing warns, and the ignored `expected_text` is why nothing can. A subsystem in "I am replaying" mode is exactly where an invariant check is most wanted and least likely to exist; assume no diagnostic will arrive from inside one and prove the property from outside it.
**Scribobulate**: `src/lineendings.rs` — `new_editor_buffer` (the choke point, the only route to an armed buffer) and the private `wire_paste_normalization`; `window::tabs::lifecycle`'s `build_tab_editor` is its single production caller. Guards, all three mutation-checked: `a_fresh_editor_buffer_is_armed_before_anything_can_populate_it` (fails with the lone `\r` intact when the arming is dropped from the constructor), `an_undo_replay_restores_the_exact_bytes_a_delete_removed` (deletes a range spanning a CRLF and asserts the precondition that it does), and `an_undo_replay_repairs_a_lone_cr_rather_than_restoring_it`, which builds the precondition-broken buffer on purpose and pins the remedy chosen — it asserts `"a\nb"` where the rejected bracket would assert `"a\rb"`. TDD §1.10.
**See**: gtk4-rs skill → controllers-and-bindings (GTK4Rs/AP-303), landed as skill commit `8fd296b` and read back from the installed copy on this host rather than taken on report; attribution there is split the same way it is here — the source-traced mechanism to this seat, the Quartz behavioural arm to the macOS one. **Two stacks, not three**: 4.6.9/X11 and 4.22.4/Quartz. The `probes/README.md` 2026-08-20 figure is the macOS seat's own earlier session, so it is the second stack again and not a third. Kin GTK4Rs/AP-73 (the content-transform layer this handler lives in) and GTK4Rs/AP-110 (an undo barrier that is missing rather than silently wrong).

## 317. A counter that stops being able to SEE its subject reports the intervention as a success
> *Non-core (general engineering discipline / measurement method) — full; no GTK internals. Do not fold into the gtk4-rs skill. Kin to #132, which is the same mistake one level up: there the GUARD's input set was not the thing it policed, here the INSTRUMENT's.*

**Symptom**: a macOS footprint investigation produced a clean, plausible attribution — one leaked `NSSavePanel` per file-chooser invocation, about half the growth recoverable by closing the abandoned panels — and **both halves were wrong**. The recovery figure was a single run that did not survive repetition, and the "0 live panels after closing them" that made it look mechanistic was a counter gone blind.

**Root cause**: the count enumerated `[NSApp windows]` for `NSSavePanel` instances. **`-[NSWindow close]` REMOVES a window from that list without deallocating it**, so after the intervention the counter reported zero because it could no longer see the object, not because the object was gone. A dealloc spy — an associated object whose `-dealloc` increments a counter — showed **zero panels deallocated in every mode tried**, including closing them and including explicitly balancing the toolkit's own retain. The intervention had never freed anything; the metric had merely stopped being able to observe the failure. The membership of the list was a property *the operation under test directly manipulates*, so the measurement returned that operation's own side effect and it was read as the outcome.

**What was tried and why each failed**: counting `[NSApp windows]` — measures list membership, not liveness, and the two diverge **exactly where the intervention acts**, which is the worst possible place for them to diverge. Reading bytes alone — cannot separate "still referenced" from "not returned by the allocator". One run per cell of a mode matrix — the *differences* between modes were noise (948.5 KB/cycle against 920–937, spreads of 6–15) while the *baseline* reproduced to 2.5% over five runs, so **the stable part of the measurement lent false credibility to the unstable part**.

**Scribobulate**: `probes/native-chooser-rss.m` grew `--track-dealloc` (a `DeallocSpy` associated object) precisely to catch this, and `--track-dealloc` against `--reap` is the reproduction. **The replacement instrument then produced the same species of artifact, which is the point**: in panel-REUSE mode the spy is re-attached each cycle under the same key, so the previous spy is released and counted — `deallocs=19` was nineteen *spy replacements*, not nineteen panels freed, in a mode where the panel deliberately never deallocates (`live_panels=1` throughout is the correct reading). A dealloc spy is only sound while the thing being re-created is the thing being counted. Third instance in one investigation, alongside a stale branch confirmation that described a snapshot rather than the current state — same shape, three different subjects. **The fourth was the dangerous one, because it failed the other way**: a reconfiguration probe reported a stale-state MISMATCH on 12/12 cycles and nearly killed a real fix, when the probe's own unconditional setters were overwriting the per-cycle configuration a few lines after applying it. The tell was an **asymmetry** — title and prompt tracked perfectly while name and directory never moved, and no plausible platform behaviour keeps two properties stale and two fresh. **A uniform failure would have looked like a finding.** False negatives get no scrutiny precisely because they confirm the pessimistic prior everyone already holds, so the discipline is to interrogate a negative result as hard as a positive one — and to treat a *partial* failure as evidence about the instrument before it is evidence about the subject. **A fifth fault sits one level up from all of them**: a probe that stood in for the production control with a bare stock AppKit widget could not see that the real object holds a raw back-pointer into a `g_free`d struct — the gap was in what the probe MODELLED, not in what it measured, and no amount of care about the instrument's *readings* would have surfaced it. When a probe stands in for production code, enumerate what the stand-in leaves out before trusting the result. **And a sixth, which is the one that reached the register — and it was the reviewer's, not the reporter's**: identifiers were carried between a version that was MEASURED and one that was READ, and the *correction* was worse than the error. Told the identifiers looked wrong, this project's own seat read a local clone, asserted it was current `main`, ran `git log -S "<symbol>"`, got nothing, and reported "never existed, across the entire repository history". The clone was on a **stable branch**, so `-S` searched only that branch's ancestry and could not see a change made on `main` after the branch point — the symbol was there all along, and the confident falsification was the least accurate of the three claims in play. **A source citation is version-scoped evidence, not a fact**: name the branch and commit with every claim, and remember that a repository-wide negative requires `git log --all -S`, because `-S` alone is scoped to whatever `HEAD` happens to be. **A seventh, from the same investigation**: an SDK header's *documented contract* was cited as evidence of *runtime behaviour* and a whole crash filing was built on it — the shipping implementation no longer enforced the sentence. A header is primary evidence for API **surface** (a `@property` attribute, an availability macro, "this class is not a singleton") and never for what the binary does; a deprecated symbol's contract in particular should be assumed stale until observed, since deprecation is exactly when an implementation gets rewritten underneath its documentation. The register entry it corrected was demoted from *attributed* to *narrowed* rather than patched, and now carries a standing rule for that issue: no byte figure without **n and spread**, and no window-list count treated as a liveness count.

**The waiting case has its own corrective, because the question above cannot reach it.** A detector whose only possible output is *"not yet"* is **indistinguishable from a broken one**, and waiting is not evidence that it is watching. MEASURED here: an unlock watcher looped on `until … | grep -q "…= 0"` where the value was printed to **stderr**, so the pipe carried nothing, the condition could never match, and it would have spun forever straight through the event it was waiting for — not late, never. It was exposed only because a *status* helper carrying the same bug printed "UNLOCKED" beside a dictionary that read `= 1`; note the direction — it claimed the gate had **cleared**, and trusting it would have produced a full run of measurements labelled with the wrong condition. **This project already had the discipline, in a test nobody had connected to it**: `codeview::markers::a11y_integration_tests::a_card_takes_the_focus_only_when_the_opener_asked_for_it` *panics on its own precondition* — "the stand-in can hold the focus (a mapped, active toplevel)" — rather than passing when no active toplevel exists, which is why a locked macOS session turns that suite red instead of green-and-meaningless. Whoever wrote it got there first. So: **anything that polls for a state change needs a POSITIVE CONTROL** — assert up front that it can read the target signal at all, print what it currently reads, and refuse to wait if it cannot; and **exercise the condition in BOTH directions** — against the real current state, which must *not* match, and against the target state, which must. A detector exercised only against the state it is waiting to leave has been tested in the one direction that cannot fail.

⛔ **AND THE TARGET-STATE SAMPLE MUST BE REAL, NOT SYNTHESISED — the version of this rule without that clause was itself insufficient, and failed within the hour.** The same watcher, "fixed" and carrying a positive control, was **inert a second time**: macOS does not set `CGSSessionScreenIsLocked = 0` on unlock, it **removes the key**, so a condition waiting for `= 0` can never match on any machine in any state. The control had been exercised against the target direction — using text its author had *written by hand*, having no unlocked machine to sample. That proves the detector can match a string you invented; it says nothing about whether the string ever occurs. **The failure was in the signal's FORM (absence versus zero), and the form is precisely what a synthesised sample assumes.** So: if you cannot obtain a real sample of the target state, **you do not have a positive control** — you have an assumption about the signal's shape, and it should be labelled as one. Corollary for the detector's shape: prefer reporting a *tri-state* (`LOCKED`/`UNLOCKED`/`BROKEN`) over a boolean or an exit code, so "cannot read the signal" is distinguishable from "signal says no".

**The documentary case has its own decay mode, and it is silent.** A note *about* an artefact — a working-memory gloss on a register entry, a reminder about a stash, a comment describing code — **decays independently of the artefact, because nothing re-reads it.** MEASURED, two notes from one seat, written the same way and both true when written: one about a regression guard's coverage survived verification against the test; one about a stash's contents was wrong in subject, owner and status, and would have sent a cleanup hunting for content that never existed. **Nothing in either note distinguished them** — only the artefacts did. So a note is a hypothesis about the thing, exactly as a source tree is a hypothesis about the binary: cite, act on, and clean up against the **artefact**, and treat any note older than the last change to its subject as unverified. **And a note that identifies its subject by POSITION is a hypothesis about the CONTAINER, which is worse** — the stash note said "leave `stash@{0}` alone, it is the only copy"; the entry it meant had long since landed, a *different* stash occupied that index, and following the note literally would have preserved the wrong object forever while protecting nothing. Containers get reused; name the subject by content. (This is the general form of POLICY's rule against citing a register entry from a remembered gloss.)

**The multi-agent case is the same fault with a person-shaped subject, and it nearly wrote a false correction into this register.** Two `researcher` instances ran in parallel on one brief, displacing each other from a single seat. This project's own seat then recapped an entire investigation to whichever one held the seat, **as if one agent had done all of it** — attributing to it a draft it never wrote, retractions it never made and an error entry it did not earn. It refused the recap and demanded locators, which was correct. **A seat NAME is a container, not an identity**, and containers get reused — exactly the by-position failure above, with an agent in the slot instead of a stash. Corollary that cost nothing and saved the filing: **do not merge two agents' findings by assuming they are one agent's** — where two independently-produced artefacts conflict, keep both, label the authorship, and have the live party review the other rather than adopt it. The two findings documents from this investigation are kept side by side for exactly that reason — **and keeping them paid**: on review, the exited instance's draft caught two defects in the surviving instance's own document (a missing `setAccessoryView:nil` before a release, which balances the books without freeing anything, and a deprecation claim checked against the `.c` but never against the header, which *had* moved), while the surviving instance found six factual errors in the draft. Neither artefact was correct alone. Merging them by assuming one authorship would have silently picked one set of errors.

**Lesson — the operational form, which is one question rather than a list.** All of the faults above failed the *same way procedurally*: **each returned a confident CLEAN result to a question it had silently narrowed.** A window-list census answered "what is in this list" when asked "what is alive". An SDK header answered "what was the contract" when asked "what does it do". A `git log -S` answered "on this branch" when asked "in this repository". A stand-in widget answered "what does a stock control cost" when asked "what does OUR control cost". None announced the narrowing, and a clean negative is exactly where nobody looks for one. So the cheap check that would have caught every one: **of every clean negative, ask what NARROWER question it might actually have answered.** Corollary, from the artefact that finally settled the branch dispute: **where a claim concerns code a user executes, prefer the SHIPPED BINARY over any source tree** — `nm`, `strings`, `otool` — because a tree is a hypothesis about the binary, and three parties quoted incompatible source at each other for an hour before anyone consulted the library that actually runs.
...
**Symptom**: a suite is clean under `G_DEBUG=fatal-criticals` on one platform and trips repeatedly on one benign assertion on another (`gdk_surface_thaw_updates: assertion 'surface->update_freeze_count > 0' failed`, on every popover open or dismiss), and the obvious way to arm the gate there is to mask that message's domain.
**Scribobulate**: `scripts/pipeline.steps` arms `G_DEBUG=fatal-criticals` on the **Linux and Windows** step-5 commands — not Linux alone, which is what this line said when it was written and what the contract had already outgrown at that commit — and declares its absence on macOS explicitly with `disarm.macos integration permanent <reason>`, so the run output states the gate is off and why rather than leaving it to a source comment; POLICY § Logging records that macOS *cannot* carry it, that the mask is refused, and what it would have cost — a latent 4.22.4 defect in the same file where a frame presented while unmapped leaks a freeze and that surface silently stops repainting. **If a macOS surface is ever reported as having stopped repainting, start here.**
...
**Symptom**: `scripts/lint-references.sh` check 12 tested Win32 path legality with `grep -qP`. On a stock Mac, BSD grep rejects `-P` and exits 2; the call sat inside `if … || grep …`, where a non-zero exit reads as "no match", so `set -e` never saw it and the check printed **PASS over every tracked path** on the one platform it was dispatched to. A gate that could not go red was indistinguishable from one that passed. Four QA reviewers found it independently by reading; nobody found it by running.
**Root cause**: two layers, and the second is the one worth carrying. (a) `-P` is a GNU extension — BSD grep 2.6.0-FreeBSD, which macOS ships as `/usr/bin/grep`, has no PCRE support, and it reports that as an *exit code* that the surrounding shell idiom silently reinterprets as a *verdict*. (b) The macOS seat's **interactive** `grep` was a shell function wrapping ugrep 7.8.4, which DOES support `-P`. So the one seat positioned to catch a BSD-grep defect would, running the command by hand, have watched it work perfectly. The gate's verdict was a property of the operator's PATH rather than of the repository — and a shell function does not cross into a script, so the hand-run and the gate were executing different programs under one name. `type -a grep` listed the function first and `/usr/bin/grep` second; `bash -c 'command -v grep'` answered `/usr/bin/grep`. Both answers were true and only the second one is what the gate runs.
**Scribobulate**: the check is now two POSIX-ERE stages behind `win_illegal_path` (character rule case-sensitive with the control range built by the shell; reserved device names as their own `grep -i` pass, since ERE has no inline flag) plus the `--self-test` corpus it shipped without — 18 cases, mirrored string-for-string into the PowerShell twin's `Test-WinIllegalPath`. Proven from Linux by running the corpus under a stub `grep` that rejects `-P` the way BSD's does: the PCRE version fails 17 cases, the rewrite passes. Then proven on the hardware: the macOS seat mutation-tested it by planting seven illegal path classes and confirming all seven named and the gate red before accepting the fix. **The generalisation, which is the reusable half**: judge a script's tooling by what the *script* resolves (`bash -c 'command -v X'`), never by the interactive shell — a disagreement between `type -a`'s entries is the tell; prefer POSIX ERE to `-P` in anything a non-GNU platform runs, which makes the class moot rather than detectable; and a gate that has only ever been observed passing has not been observed, because a green run on a clean tree was precisely this bug's output.
**See**: project-specific tooling; the predicate and its corpus live in `xtask/src/lint/patterns.rs` (`win_illegal_path`) and `xtask/src/lint/corpus.rs`. The two shell ports the episode happened in are retired, and with them the `grep -P` portability hazard itself — the predicate is now Rust, so no platform's shell decides what it can express. Content authored by the macOS seat and MEASURED there 2026-08-21 (macOS 25 Darwin.0.5, aarch64; `/usr/bin/grep --version` = "grep (BSD grep, GNU compatible) 2.6.0-FreeBSD"; `echo x | /usr/bin/grep -qP x` → exit 2, "invalid option -- P"; ugrep 7.8.4 reports `-P:pcre2jit`); landed here by the register's owning seat, edited for format and not for argument. Kin: ScrAP-318 and ScrAP-268 (a gate disarmed rather than absent), ScrAP-207 (two ports that agree by comparison instead of by derivation).

## 320. The unresolvable pointer — a reference whose target the reader cannot dereference, mistaken for a delivery
**Symptom**: both parties believe an artefact was delivered; neither has it, and the belief is stable for hours. The tell is a sentence shaped *"it's ready — see `<path>` / the earlier message / the doc"*, where the target resolves in the SENDER's context and was never checked in the reader's. Nothing bounces and nothing errors, because **sending** succeeded — it is **dereferencing** that failed, and no channel reports that.
**Root cause**: the sender validates the pointer against their own context, which always succeeds, and mistakes that for validating delivery. Three boundaries, all silent: **filesystem/seat** (`~/...`, `/tmp/...` — not a shared namespace when the reader is another machine or clone), **time/TTL** (a ToasterTalk brief past its TTL is *gone, not queued*), and **visibility/VCS** (gitignored, unpushed, or on an unshared branch). A fourth, identity, is the same error with authorship as the namespace: handing an agent a parallel instance's history as its own.
**Scribobulate**: five incidents in one session, across two agents. The load-bearing one for generality is **this seat's**, not the researcher's: `docs/` is gitignored (`.gitignore:16`), and both platform seats were dispatched their QA work by being pointed at `docs/code-review.md`, which reaches neither clone. It happened within an hour of the researcher pointing this seat at a 14.5 KB issue body on *its own* filesystem, and neither had told the other. **Two independent agents, same failure, same hour** — which is what makes it a property of the medium rather than one agent's lapse. The others: a research brief that TTL'd and was answered from a later message's reconstruction; two status reports claiming "delivered" over a send that had failed on a displaced session token; and the identity case above.
**The rule — disk first, room second, and the room message stands alone.** Persist before announcing; carry enough content in the announcement to be useful if the pointer resolves to nothing (inline a short result entirely, and for a long one the verdict plus the decisive evidence); treat a pointer as a convenience and never as the payload. Three questions before sending a reference: is this path in a namespace the reader **shares**; can the target **expire**; and would the message still be useful if the pointer resolved to nothing? A "no" to the third means it is not finished.
**A SECOND INSTANCE, INSIDE THE PROJECT'S OWN GIT TOPOLOGY, and it refines the corrective.** Three completed Mediums sat on a pushed seat branch for several rounds while both seats believed them landed: the seat reported them closed in a round summary written from memory, and the integration seat merged four other branches by name in the same period without once running `git branch -a`. Neither claim was dishonest and neither was checked. **The corrective is not "the integrator should list branches" — it is that DELIVERY NEEDS A CHECK ON BOTH SIDES**, because one side said "pushed", the other said "merged", and nothing asked the repository. Concretely: before reporting an item closed, confirm the artefact is in the INTEGRATION branch by path or symbol rather than by remembering you pushed it; and before deleting a branch, confirm it is an ancestor of the integration branch rather than trusting the merge that was announced. Running that check immediately found a second instance — a branch still on the remote after its merge, in a state both seats had already claimed twice.
**Correctives, in preference order**: (1) inline the content — the cost is bytes against hours; (2) verify resolvability at the far end before treating it as delivered — **the Windows seat modelled this correctly**, running `git cat-file -e <ref>:docs/code-review.md` across every remote ref and reporting the file ABSENT rather than assuming a bad fetch, which converted a silent failure into a fixed one inside a minute; (3) mark non-channels as such, as `docs/code-review.md` now is in its own header.
**Why it is hard to see**: the sender's evidence is always positive. They can open the file; the send returned an ID; nothing in either local view is wrong. The error lives only in the **gap between two contexts**, which neither party can observe alone. That is why it recurred five times among agents who spent the same session verifying everything else scrupulously — **verification discipline applied to claims does not automatically extend to references.** A reference feels like plumbing. It is a claim: *you can get this from there*.
**See**: project-specific process. Family — the **silent negative**: ScrAP-317 (a counter that stopped being able to see its subject), ScrAP-319 (a null result that read as a verdict because the instrument disagreed with the subject). An unresolved pointer is a silent negative *in the delivery channel* — nothing failed loudly, so both parties inferred success from the absence of an error, and the counter-discipline is the same one: make the negative loud, or verify the positive. Content authored by the `researcher` seat and MEASURED there across five directly-observed incidents; the generality claim beyond this session and toolchain is INFERRED from five instances, two independent agents and three boundary types, and no survey of other rooms was attempted. Landed by the register's owning seat, edited for format and not for argument.
...
**Symptom**: a mutation run reports a green baseline and every mutant killed; coverage looks total. One or more of those kills is the HARNESS failing rather than the suite detecting anything, and the row reads identically either way. The tell is a kill whose reported reason does not name the assertion that should have fired — generic text, a path, or the same string across mutations that break unrelated things.
**Root cause**: the harness infers "killed" from a PROXY — the child's exit code — rather than from the suite reaching its own conclusion. Every way of stopping a suite early yields a non-zero exit: a parse error in the mutated text, a wrong working directory, a missing fixture, a path the subject resolves from its own location rather than the harness's. All of them score as detection. **The harness's own breakage and a real kill are the same symbol**, so a broken harness reports the strongest possible result.
**The direction is the whole finding.** This errs OPTIMISTICALLY. A mutation run that lies pessimistically wastes an afternoon; one that lies optimistically retires a gap you then stop looking for. **A mutation run you trust is worse than none if it can only fail upward** — and mutation testing is what POLICY names as the only admissible evidence that a gate can fail, so this defect corrupts the project's court of last resort.
**What was tried and did not work**: reading the exit code more carefully — the exit code is not the problem, it is a faithful report of a process that stopped, and *stopping is ambiguous*; inspecting the kill count — 21/21 is the shape of both success and total harness failure; trusting a green baseline — necessary, not sufficient, since the expensive instance had one.
**Correctives, in preference order**: (1) **judge KILLED on the suite's own terminal statement** — its final success line, its own verdict — never on the exit code alone; require both signals and print INCONSISTENT rather than a verdict when they disagree, so the ambiguous case is loud instead of optimistic. (2) Always run the UNMUTATED text through the identical path and print the result; a red baseline voids every row below it. (3) Require each kill's reason to NAME the assertion that fired — a generic reason, or one shared across mutations breaking unrelated things, is a harness artefact until shown otherwise. (4) Run the mutant in a faithful copy of the subject's environment whenever the subject resolves paths from its own location. (5) Lift the subject's text VERBATIM, by AST extent rather than retyped, so the harness drives the program the gate runs and not a re-implementation of it.
**THE MIRROR IMAGE, measured within the hour by the same seat, in a harness written AFTER this entry.** A bash harness for another file's self-test never verified the mutation had APPLIED: in Perl, `\Q…\E` applies quotemeta to the RESULT of interpolation, so every anchor containing a shell `$var` had that variable interpolated to empty — leaving a complete, well-formed and entirely plausible-looking pattern (`if [ -z "" ] || [ "" = "" ]; then`) that simply matched nothing. It did not become a degenerate pattern someone might notice; it kept the right shape, the right length and the right look at a glance, and lost only its content. That is what let five rows pass review. The unmutated text then ran green and was scored **SURVIVED** — five of twelve rows void, and one further row disabled a different check than its label claimed because a non-global `s///` hit the first of two identical `permanent|tooling)` arms. So the same missing guard produces a **false survival**, which invents a gap instead of hiding one. That is the PESSIMISTIC direction: it wastes an afternoon and, worse, it can send a false finding to another seat about their work — here it wrongly reported a pinning table as uncovered when the table was real, working coverage. Cheaper than the optimistic failure and the same root cause, so the corrective is one rule, not two: **a mutation row is void unless its anchor matches the subject EXACTLY ONCE.** Zero matches means it never applied; more than one means you do not know which site you changed.
**The weaker form of that rule is not enough, and the two harnesses above are the proof.** "Void until the harness proves the mutation reached the subject" was *exactly* the bash guard — mutated text ≠ original — and it PASSES the second bug in this paragraph: a non-global `s///` hit the first of two identical `permanent|tooling)` arms, the text genuinely changed, the mutation genuinely reached the subject, and the row was still void because it disabled a different check than its label named. Measured: that row returned SURVIVED with no flag on it. The PowerShell guard required the anchor to match exactly once and reported `ANCHOR MATCHED {n}x -- not applied` otherwise, which catches both failures — non-application at zero, ambiguity above one. One clause stronger, and it is the clause the bash port dropped. The same seat's PowerShell harness had that guard and its bash one, written an hour later, did not — a discipline is not transferred by having learned it once in another language.
**A THIRD ROUTE TO THE SAME MEANINGLESS GREEN, adjacent rather than inside: a test that shares an IDIOM with its subject can be switched off by one edit to that idiom.** MEASURED here on the harness written to close this file's coverage gap: it accumulated its own verdict with `errs=$((errs + 1))`, the very construct a file-wide mutation removes, so neutralising every error counter in the file disarmed the test AND its subject together — fifteen "NEGATIVE CASE NOT REJECTED" lines printed and the harness exited **0**. It failed in the direction that reads as success. A harness is the worst possible address for this, because nothing downstream re-checks the checker. Corrective: the verdict must not be computed by anything the subject also uses — here, accumulating case NAMES into a string and testing emptiness rather than counting. Found only because the mutation was run file-wide rather than against the single function under test; a targeted stub could not reach it.
**A FOURTH ROUTE, and the one most likely to recur: a SOURCE-READING check whose pattern matches its own source.** MEASURED: a self-test asserting that a runner's `uname` guard sits after its argument-parsing loop located the guard with `grep -n 'macOS only'` — and that string also occurs *inside the checking function itself*, so the check found its own text, reported the guard at line 90 instead of 170, and failed a correctly ordered file. Note the direction: it failed toward RED **by luck of which line came first**. Had the guard sat above the checking function, the self-match would have returned a larger line number and the check would have PASSED a broken file. Every source-level check is exposed to this, and the direction is decided by file layout rather than by anything principled.
**Two correctives, and which applies depends on whether the check can avoid its own file.** (a) **Exclude the checker from its own input set** — this is how the project's existing source-level tier is already safe, and it is worth knowing because nothing said so: `lint-references` check 5 hunts a banned attribute over the scan set's `src/*.rs` members, which do not contain the gate, and checks 1, 6a and 8 explicitly drop the gate's own corpus file from their input because it quotes the fixtures they hunt for. The protection is input-set narrowing, not luck. (b) Where the check must read its own file *because the property is about that file* — as an ordering assertion necessarily is — exclusion is unavailable and the answer is to **anchor so the checker's own text cannot match**: at column 0, where the subject is a top-level statement and every mention inside a function is indented. A check that then cannot find its subject must report INCONCLUSIVE and fail, never pass.
**Why it is hard to see**: the harness's evidence is positive in exactly the direction you want. Green baseline plus all-killed is the result you were hoping for, so it survives the glance that catches a surprising answer. And it has no independent witness — a mutation harness is the instrument that certifies other tests, so nothing downstream disagrees with it.
**Scribobulate**: three instances, one seat, one session, on Windows PowerShell 5.1. (1) A check-12 predicate harness wrote mutants to a scratch directory while the script resolves its scan set from `$PSScriptRoot`, so every mutant died on a missing `.scan` and five of six clauses reported "killed" — the BASELINE died too, visible only because it was printed. (2) The same mistake against `packaging/windows/pipeline.ps1`, which resolves its contract two directories up; fixed by running each mutant inside a mirror of the repo layout. (3) The expensive one: baseline GREEN, 21 of 21 killed, one of them false — deleting the `-SkipIntegration` accumulation was scored as detected, and re-running that mutant directly gave exit 0 and the full self-test output. **The harness was right twenty times out of twenty-one, which is precisely why the row was believed.** The gap it hid was real: the provenance cases set `$script:OverriddenSteps` by hand, proving the warning PRINTS and saying nothing about whether anything fills the variable it reads. The durable corrective in-tree is `Show-OperatorSkip`, which exists as a function rather than four lines inline in the run loop solely so `-SelfTest` can reach the recording half of that feature — and the reusable shape is **code inside a runner's main `try` block is invisible to a self-test dispatched before it**.
**See**: family — the **silent negative**, in its optimistic direction. ScrAP-317 (a counter that stopped being able to see its subject), ScrAP-319 (a null result that read as a verdict because the instrument disagreed with the subject), ScrAP-320 (a pointer the reader could not dereference). Those three are checks that could not go red; this one WENT red for the wrong reason and thereby certified something — the same error with the sign flipped. Near sibling one level down, the **vacuous assertion**: an execution case asserting `synthetic-probe-ran` against a step that echoes its command line *before* running it, which passed with the executor gutted (measured, same session; the corrective is to assert a value the command's execution PRODUCES and that appears nowhere in the command text — `SCRIB_A=123 SCRIB_B=456 set /a %A%*%B%` asserting 56088). A vacuous assertion passes for the wrong reason, a spurious kill fails for the wrong reason, and both convert *the instrument agreed with me* into *the property holds*. Content authored by the Windows seat and MEASURED there across three directly-observed instances on Windows PowerShell 5.1; the generality beyond that toolchain is INFERRED — the proxy-for-conclusion mechanism is not PowerShell-specific, but no other harness was surveyed. Landed by the register's owning seat, edited for format (the two implementation fields collapsed into one) and not for argument.

## 322. A control is a property of a CLAIM, not of a probe — and one control makes every other claim feel covered
**Symptom**: a probe carrying a deliberate, genuine control reports several findings, and the ones the control does not actually cover are wrong. Nothing looks careless: the probe HAS a control, its author knows why, and the uncontrolled claims sit in the same output as the controlled one and read with the same authority.
**Root cause**: **the instrument was the independent variable and nothing varied it.** MEASURED — `probes/textview-primary-overwrite.c` carried `--trap` against `--relinquish`, a real control for "does the relinquish hook clear our claim". On the strength of it two further numbers were reported: "THREE selection emissions, not two", and "the claim and relinquish halves race through a transient no-selection state". Both were false. There are two emissions and there is no transient; both figures were manufactured by the probe's own `gdk_clipboard_set_content`, which destroys the buffer's selection (`gtk_text_buffer_content_detach`, `gtktextbuffer.c:272-292`, located from source after the probe measured the effect). EVERY mode of that probe touched the clipboard, so no mode could answer "does the handler perturb the subject at all".
**Why the credibility transfers, which is the whole trap**: a probe that HAS a control FEELS controlled, and the feeling is strongest exactly when the probe is well built in one dimension. The two uncontrolled claims inherited the credibility of the controlled one, from readers including its author.
**What was missing**: two modes that change nothing — a `--baseline` with no handlers at all, and an `--observe` with the handler installed and never touching the clipboard. Adding them inverted the conclusion in a single run: both showed 2 emissions and a surviving selection against the instrumented mode's 3 and a destroyed one.
**Cost, and it is why this has a number when several sibling findings this round did not**: the false results were sent to ANOTHER seat, which revised a design that was not broken and accepted blame for a defect that did not exist. Caught one turn later, before it was acted on — but the failure mode is a seat acting on another seat's bad measurement, which is the expensive half and was one turn away.
**Correctives, operational rather than "be careful"**: (1) for EVERY number reported, name the mode that differs from it by exactly one thing — if you cannot name one, you are reporting an anecdote and not a measurement; (2) every probe needs an arm in which the instrument does nothing, because a probe with no untouched subject cannot separate its own effect from the behaviour it is measuring; (3) **a control belongs to the CLAIM** — adding a claim to an existing probe adds a control obligation and does not inherit the one already there.
...
**See**: project-specific method. Kin — ScrAP-321 (a green that means nothing, by several routes) is related and distinct: 321 is a CHECK that cannot fail, this is a MEASUREMENT that measures the meter. ScrAP-320's family one level up: a thing that felt like plumbing turned out to be load-bearing evidence. Content authored by the macOS seat and MEASURED there 2026-08-21 (GTK 4.22.4/Quartz, macOS 25 Darwin.0.5, aarch64); the GTK mechanism traced from source by the researcher. Landed by the register's owning seat, edited for format and not for argument.
...
> *Compressed: the GTK mechanic is core-GTK and now lives in the `gtk4-rs` skill as **GTK4Rs/AP-317** (widgets-and-composites, landed `1be3d4c`), cross-linked there with GTK4Rs/AP-105 and GTK4Rs/AP-307 as the "kept its natural size, three mechanisms" family. What stays here is where this project relies on it and the discipline lesson that produced it.*

**Symptom**: a guard written to prove that the sidebar's divider cannot hide a section — `assert!(section.height() >= FLOOR)` after dragging to each extreme — **passed under its own mutation**. With `shrink_start_child`/`shrink_end_child` flipped to `true`, which is exactly the defect the guard exists to catch, the crushed section still measured its full ~110px. Nothing warned; the guard read as thorough and was inert.
**Root cause**: GTK does not shrink a squeezed `GtkPaned` child, it hands it its NATURAL size and shifts it out of the pane, while separately marking it child-invisible — so the allocation, and therefore `height()`, stays healthy-looking. Assert instead on the quantity the mechanism CLAMPS: `position`. Full source trace and the corrected oracle are in GTK4Rs/AP-317.
**Scribobulate**: `window::gtk_integration_tests::the_divider_cannot_crush_a_sidebar_section_away` asserts the paned's `position()` against the sidebar's own height, at both extremes and for both sections (TDD 20.21). It is what keeps the drag from contradicting the four-state rule (TDD 20.9), where `win.outline`/`win.annotations` are the single source of truth for whether a section is on screen — a section crushed to nothing would be gone while its action still reported it shown. Mutation-checked: `shrink=true` fails it with `position 0 of 511`; the `height()` form did not.
**Lesson**: **a guard is not evidence until it has been shown to fail**, and the run that shows it is also the run that tells you whether you measured the right thing. This one was written, read, and believed by its author first; only the mutation exposed that it was aimed one surface away from its own subject. Kin — ScrAP-119 (the other `GtkPaned` trap this same divider had to answer, its capture-phase handle inflation).
