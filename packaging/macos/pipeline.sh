...
#
# It DERIVES every step from scripts/pipeline.steps rather than restating them — the
# same design as scripts/pipeline.sh (Linux), which this file mirrors deliberately
# rather than sourcing: three thin per-platform ports reading one contract, the same
# shape as lint-references.sh/.ps1. `derived_step_ids` is the ONLY producer of the
# ordered step list, and BOTH `--list-steps` and the run loop consume it, so there is
# no second list that could disagree. `--self-test` proves that property still holds
# after an edit; it does not create it.
#
# This runner's per-platform command bodies (cmd.macos.* in the contract) are what
# differ from Linux, not the parsing/execution logic below:
...
run_step() {
    ...
    # packaging is opt-in: skipped unless --package is given, required when it runs.
    # A SKIPPED packaging step is still ANNOUNCED, never omitted — the same reason a
    # non-applicable step is declared rather than left out: an omission the reader
    # cannot see is how a step quietly stops existing.
    if [ "$class" = "packaging" ] && [ "$PACKAGE" -ne 1 ]; then
    ...
    # override skipped a required gate, but the run must SAY SO — otherwise an installer
    # built from an ungated tree looks exactly like one that passed everything. Overrides
    # only, never contract-declared non-applicable steps: those are always absent on their
    # platform, so naming them every time is noise rather than news.
...
# The libtest arguments that APPLY the carve-outs, so the run does what the announcement
# says. Announcing without applying is worse than silence: while the list is empty nothing
# is mis-reported, which is exactly why the gap stays invisible until the first carve-out
# is added and the run prints `skipped: <test>` for a test it ran.
carveout_skip_args() {
    local name args=""
...
# Append past a `--` separator, reusing one if present: a second `--` reaches libtest as a
# literal argument rather than a separator.
apply_carveouts() {
    local cmd="$1" skips="$2"
