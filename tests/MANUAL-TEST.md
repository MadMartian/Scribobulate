...
- [ ] **18.21** Under **Synthwave**, render a document with headings at every level (`#` … `######`) → the **h1 is neon magenta** and h2–h6 are the sunset gold, so a level the theme states wins and a level it leaves empty falls back to `heading_color`. Put a link inside the h1 → the link keeps the **link colour**, not the heading's (the link tag is higher priority). Then add `heading_colors = ["#00ff00", "", "", "", ""]` and `heading_fonts = ["Courier, monospace", "", "", "", ""]` to `[themes.sepia]` in `~/.config/scribobulate/themes.toml` and relaunch → Sepia's h1 turns green and monospace while h2–h6 keep the body colour and face (**the user-file merge — a key missing from `overlay`'s `take!` list works for built-in themes and silently drops every user override**). Then Export ▸ HTML the same document → view source: the `h1` and `h2` rules carry those same two colours. Switch to System → every heading is the plain body colour and face, exactly as before this rubric existed
- [ ] **18.22** Under **Synthwave**, render a document with headings at every level → each heading carries a **neon-magenta rule under its text** and sits in visibly more space than before, with the gap tapering from h1 to h5. Zoom in (Ctrl++) → the gap grows **with** the zoom (it is a design-time px metric, not a Pango scale). Then add `heading_overline = "single"` to `[themes.synthwave]` in `~/.config/scribobulate/themes.toml` and relaunch → a second rule appears **above** each heading, in the heading's own gold (there is deliberately no `heading_overline_rgba`; ⚠️ if you find one, that is a heap double-free on GTK 4.6, not a feature). Try `heading_underline = "wavy"` and `"double"` → the line under the text changes shape. Try `heading_underline = "zigzag"` → the app starts, the rule simply disappears, nothing breaks. Then Export ▸ HTML → view source: the `h1` rule carries `text-decoration-line`, `text-decoration-style` and `margin: calc(...)`. Export ▸ PDF → the headings carry the same rule (the PDF resolves against System, so put the keys in `[themes.system]` to see it). Finally switch to System → no rule, no extra space, headings exactly as before this rubric existed. **Scope limits, not defects:** `heading_space_above`/`_below` do not reach the PDF at all, and in the HTML export a theme with BOTH rules on tints the overline with the underline colour (CSS carries one `text-decoration-color` per element)
