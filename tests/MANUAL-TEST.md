...
- [ ] **7.12** Right-click any tab → context menu with Save / Save As / Close Tab / Close Other Tabs / Move to New Window / Copy Full Path / Reload / Rename, each acting on THAT clicked tab (not necessarily the active one); Close Other Tabs is disabled when it is the window's only tab, and Move to New Window is disabled under the same single-tab condition View ▸ Move Tab to New Window already uses (TDD 7.12). Save on a right-clicked INACTIVE dirty tab writes THAT tab, focuses it, and leaves the other tab's file untouched (TDD 15.19); Save on a right-clicked clean tab (backing file present) is greyed, Save As never is (TDD 15.20)
- [ ] **7.12a** Tab context-menu access keys (TDD 7.13, ScrAP-70): each row shows one letter underlined (**S**ave, Save **A**s, **C**lose Tab, Close **O**ther Tabs, **M**ove to New Window, Copy **F**ull Path, **R**eload, Re**n**ame); with the menu open, pressing that bare letter (no Alt) invokes the row. `S`/`A`/`C`/`M`/`F`/`R`/`n` match their File/View-menu counterparts. A disabled row (e.g. Close Other Tabs on a single-tab window, or Save on a clean tab) ignores its key.
...
- [ ] **7.23** **Linux: a second `scribobulate` on PATH is reported, and the install still completes (TDD 7.23).** *Linux half; the macOS half is 7.23m and behaves differently on purpose — there a second copy is always a mistake, here a distro package alongside a user-local build is an ordinary supported arrangement, so this WARNS and never refuses.* Run `packaging/linux/install.sh` under four planted PATH configurations and read the tail of its output; every one must exit **0**. **(a) Clean** — nothing named `scribobulate` on PATH but the installed one → the tail is **silent**; a check that chatters on a clean machine gets ignored on a dirty one. **(b) A real binary EARLIER in PATH** → `WARNING: typing 'scribobulate' does not run what was just installed`, naming **runs** and **installed** as two different paths. **(c) A copy LATER in PATH** → a `NOTE` — and **the paths must be listed under the heading**. An empty list under a populated heading is the regression on record: the list was built by a command substitution, which strips the trailing newline, so `read` returned false on the final line and the loop never ran. A heading with nothing under it is the failure; do not read the heading alone as a pass. **(d) A DANGLING symlink earlier in PATH, with a real stale binary after it** → the `DANGLING` section names the link, **and** the `runs` path is the **stale binary, not the dangling link**. That is the whole mechanism: a dangling link is not executable, so the shell skips it silently and resolution lands further down — naming the link as the winner would be a plausible-looking wrong answer. Machine-drivable: plant the decoys in temp directories and prepend them to `PATH`; no GUI needed.
- [ ] **7.23m** **macOS: the install refuses rather than adding a second bundle, and nothing it installs resolves into `target/` (TDD 7.23).** *macOS only — the mechanism is this platform's: `CFBundleIdentifier`, not the path, is the app's identity, so a second bundle anywhere is a second Launch Services registration for the same app.* **Gate 1** — with a `Scribobulate.app` already in `/Applications`, run `packaging/macos/install.sh` → **exit 1**, naming the path found and the exact `rm -rf` to clear it, **and the release build never starts** (the `Building Scribobulate.app` banner must appear **zero** times — a gate that refuses only after spending the build has failed its purpose). It must **report, never delete** a bundle it did not install. **Gate 2** — with a dangling `scribobulate` symlink in a directory earlier on PATH than `$(brew --prefix)/bin` → **exit 1**, build banner zero; in the same run a copy *later* on PATH is demoted to a `NOTE` rather than an error. **Then the anchor**, with both gates satisfied: the install completes, the bundle lands at `~/Applications/Scribobulate.app`, its signature still verifies after the copy, and **`target/macos/Scribobulate.app` is gone** — it is removed once the anchor verifies, because a bundle in a build directory registers with Launch Services like any other and would be a second candidate for the identity. **Now the regression this all exists for**: `rm -rf target/macos/Scribobulate.app` (or `cargo clean`) → `command -v scribobulate` still resolves, the target is still executable, and `man -w scribobulate` still resolves. Before the anchor, all three broke here **silently** — the links dangled, the shell skipped them, and a four-month-old binary elsewhere on PATH ran instead with no diagnostic. **Uninstall in the trap order** is 7.21m. **Linux/Windows: not applicable** — the Linux half is 7.23.
...
- **Drag-selection — `cliclick`'s `dd:`/`m:`/`du:` does NOT extend a selection in this
  ...
  primitive that reports success while delivering nothing (GTK4Rs/AP-245, ScrAP-300).
...
- **…but the MENU BAR is now fully AX-driveable, and it is the best handle on this
  ...
  Two cautions: dismiss an opened menu with `key code 53` before the next step, or it
  keeps the event grab; and a menu item's *name* is the elided label, so match on
  `"Save As…"` with the real ellipsis character, not `"Save As..."`.
...
> ScrAP-209 shape (a guard whose setup prevents the thing it guards from existing).
>
