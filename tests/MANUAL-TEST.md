...
- [ ] **18.18** Add `bold_weight = 900` and `supsub_scale = 1.4` to `[themes.sepia]`, then under Sepia render a document with **bold** text and H~2~O/x^2^ both in body prose and inside a table cell → the cell's bold/sup/sub match the body's exactly (same heaviness, same size bump). Then File ▸ Export ▸ PDF the same document → the PDF's bold/sup/sub also match (open in a PDF viewer and compare beside the preview)
