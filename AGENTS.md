...
- [`sdd/PLAN.preview-decoration.md`](sdd/PLAN.preview-decoration.md) — scoped, **not approved** (2026-08-18): externalising preview *decoration* — glyphs, bands, shapes, graphics — to the reading theme, which today owns colour, typography and decoration metrics only. **Read it before adding any theme key, and before proposing any new preview decoration**: it carries the per-component scope table (what each rendered element can support, by which mechanism, at what cost), the measured capability ceilings of all three mechanisms at the 4.6 floor, the exact edit path a new key and a new drawn decoration each take (including the silent empty-gate trap in `snapshot_layer`), and one blocking decision — POLICY currently bounds a theme to a decoration's *metric*, never *what is drawn*, so the feature as requested needs that rule amended before any of it is buildable.
