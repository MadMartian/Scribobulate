...
- [`sdd/PLAN.crash-recovery.md`](sdd/PLAN.crash-recovery.md) — **fully implemented; awaiting the operator's word to retire.** Crash recovery ("swap files") so an unclean exit no longer discards unsaved buffers: `src/swapfile/` (display-free core), `src/window/swap.rs` (write edge + the dirty↔swap invariant's choke point), `src/window/swaprecovery.rs` (startup pass). The contract is [TDD §22](sdd/TDD.md), the format is [SCHEMA.md](sdd/SCHEMA.md), the surfaces are [CAM.md](sdd/CAM.md) rows 8/10. **Read the plan before touching the snapshot write path**: it records why `replace_contents_async` was adopted and then *reversed* — that call destroys the previous snapshot on an ordinary disk-full (ScrAP-232), and the shipped mitigation leans on a specific GLib branch that a regression test pins.
