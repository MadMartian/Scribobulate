...
- [`sdd/PLAN.document-rename.md`](sdd/PLAN.document-rename.md) — **implemented** (2026-08-15), awaiting retirement: renaming the file backing an open tab, same-directory and name-only. The feature ships (`src/window/rename.rs`, `src/docio/rename.rs`, TDD §24); the plan survives only because two things in it have no other home yet — the platform claims still SOURCE-TRACED or INFERRED for want of a **macOS** seat (Windows' half was measured 2026-08-16), and the Document-Identity back-sweep's two **unverified** hypotheses about existing Save As defects. **Read it before changing anything that re-points a document's path** — Save As, the first save of an untitled buffer, or `app::attach_file_backing` — **and before writing any code that renames a file through GIO or cancels a `gio::FileMonitor`**: it records, source-traced, that **no GIO primitive refuses an existing rename destination atomically** (both are `g_lstat` + plain `g_rename`; Windows always passes `MOVEFILE_REPLACE_EXISTING`), and that a rename delivers **three** events on the old monitor on Linux/Windows — which is why the save path's self-delete guard is *insufficient* here and `g_file_monitor_cancel()` (a hard barrier, measured) is the sole mechanism. **Also before trusting any name GIO hands back after a rename**: neither `set_display_name`'s returned `GFile` nor a `query_info` reads the directory, so on a normalising volume the app can key everything — including the re-attached monitor — on a name no entry holds (ScrAP-270), and the `id::file` identity scan that fixes it is not unique per entry (ScrAP-271). The testing lessons are ScrAP-269 and ScrAP-272.
