...
- [`sdd/PLAN.build-pipeline-ci.md`](sdd/PLAN.build-pipeline-ci.md) — in progress, and widened (2026-08-13) to its actual deliverable: **an installable artefact for each OS, built when a release is published**. Three stages, in order, each retiring the risk the next would carry blind — `execute-macos`, then `execute-windows`, then packaging for all three. Read it before adding any job. It records why execution comes first (a packaging job on a platform whose build has never run in CI is untested twice over), that the original "macOS is hardest" ordering is backwards for execution (the `codesign` cost is step 10, opt-in and absent from a default run), the provisioning detail for both platforms, the unsigned-artefact decision and the gap it leaves against step 10's stated intent on macOS, and the three rules any new job inherits: invoke the runner and name no step, show the gate failing before trusting it, and verify the artefact as an artefact rather than as an exit code.
...
- [`sdd/PLAN.document-rename.md`](sdd/PLAN.document-rename.md) — **implemented and tested** on all three platforms (2026-08-16), **fully distilled; retirement deferred to a future commit**: renaming the file backing an open tab, same-directory and name-only. Nothing durable is left in it — the file carries a table naming where each part went, and is retained only because a human confirms a plan's retirement. **Do not read it for instruction; read the destinations.** The contract is TDD §24, the checks are `tests/MANUAL-TEST.md` §24, the surfaces are [`CAM.md`](sdd/CAM.md), and the GIO mechanisms are the `src/docio/rename.rs` module header. **Read that header before writing any code that renames a file through GIO or cancels a `gio::FileMonitor`**: it records, source-traced, that **no GIO primitive refuses an existing rename destination atomically** (both are `g_lstat` + plain `g_rename`; Windows always passes `MOVEFILE_REPLACE_EXISTING`), and that a rename delivers **three** events on the old monitor on Linux/Windows and **two** on macOS/kqueue — which is why a self-delete guard that consumes one event is insufficient and `g_file_monitor_cancel()` is the sole mechanism. **Also before trusting any name GIO hands back after a rename**: neither `set_display_name`'s returned `GFile` nor a `query_info` reads the directory (ScrAP-270), and the `id::file` identity scan that fixes it is not unique per entry (ScrAP-271). The lessons are ScrAP-269 through ScrAP-272 and ScrAP-274.
